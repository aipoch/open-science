...
program = Path(__file__).with_name('kernel.py').read_text()
bridge = {}
exec(compile(program, str(Path(__file__).with_name('kernel.py')), 'exec'), bridge)
import cellxgene_census as api
import tiledbsoma
bridge["tiledbsoma"] = tiledbsoma
...
class Contract(unittest.TestCase):
    ...
    def test_tiledb_uses_command_proxy_and_trust_bundle(self):
        ...
        self.assertEqual(config['ssl.ca_file'], '/fixture/ca.pem')
        ...
        with patch.object(api, 'get_census_version_description') as resolve, patch.object(api, 'open_soma') as opened:
            for args in requests:
                with self.subTest(args=args), self.assertRaises(ValueError):
                    bridge['_query']({'action': 'query_cells', **args})
    ...
    def test_dataset_search_projects_available_release_columns(self):
        frame = pd.DataFrame({'dataset_id': ['a'], 'dataset_title': ['Liver atlas']})
        census = {'census_info': {'datasets': SimpleNamespace(read=lambda: SimpleNamespace(
            concat=lambda: pa.Table.from_pandas(frame)))}}
        result = bridge['list_datasets'](census, {'query': 'liver'}, 'older-release')
        self.assertEqual(result, {'census_version': 'older-release', 'total': 1,
                                 'datasets': [{'dataset_id': 'a', 'dataset_title': 'Liver atlas'}]})
        with self.assertRaisesRegex(ValueError, 'searchable dataset metadata'):
            census['census_info']['datasets'] = SimpleNamespace(read=lambda: SimpleNamespace(
                concat=lambda: pa.table({'unknown_column': ['liver']})))
            bridge['list_datasets'](census, {'query': 'liver'}, 'older-release')
    ...
    def test_each_call_closes_census_on_success_and_failure(self):
        ...
        with patch.object(api, 'get_census_version_description', return_value={'release_build': '2025-11-08'}), patch.object(api, 'open_soma', side_effect=opened), patch.dict(bridge, query_cells=lambda *a: {'ok': True}):
            bridge['_query']({'action': 'query_cells', 'tissue': 'liver'})
            with self.assertRaises(ValueError): bridge['_query']({'action': 'invalid'})
        ...
        for action in ('list_datasets', 'query_cells'):
            with self.subTest(action=action):
                ...
                with patch.object(api, 'get_census_version_description', return_value={'release_build': '2025-11-08'}) as version, patch.object(api, 'open_soma', side_effect=opened), patch.dict(bridge, {action: read}):
                    self.assertEqual(bridge['_query']({'action': action, 'tissue': 'liver'}), {'release': '2025-11-08'})
    ...
    def test_persistent_checksum_failure_stops_after_one_retry(self):
        ...
        with patch.object(api, 'open_soma', side_effect=opened) as op, patch.dict(bridge, query_cells=read):
            with self.assertRaisesRegex(bridge['tiledbsoma'].SOMAError, 'Response checksums mismatch'):
                bridge['_query']({'action': 'query_cells', 'tissue': 'liver', 'census_version': '2025-11-08'})
    ...
    def test_checksum_failure_during_open_also_gets_one_fresh_attempt(self):
        ...
        with patch.object(api, 'open_soma', side_effect=[error, nullcontext(None)]) as op, patch.dict(bridge, query_cells=lambda *a: {'ok': True}):
            self.assertEqual(bridge['_query']({'action': 'query_cells', 'tissue': 'liver', 'census_version': '2025-11-08'}), {'ok': True})
        self.assertEqual(op.call_count, 2)
