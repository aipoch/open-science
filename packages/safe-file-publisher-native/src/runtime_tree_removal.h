...
int RemoveTreeAt(int parent, const char* name, unsigned depth, dev_t device,
  ...
  RemovalFd directory(openat(parent, name, O_RDONLY | O_DIRECTORY | O_NOFOLLOW | O_CLOEXEC));
...
napi_value RemoveAnchoredTree(napi_env env, napi_callback_info callback) {
  ...
  if (anchor.value == INVALID_HANDLE_VALUE || !TreeDirectory(anchor.value))
    return ThrowError(env, "Could not anchor runtime storage.", WindowsErrorCode(GetLastError()));
  ...
  if (error && error != ERROR_FILE_NOT_FOUND && error != ERROR_PATH_NOT_FOUND)
    return ThrowError(env, "Could not safely remove runtime tree.", WindowsErrorCode(error));
#else
  RemovalFd anchor(open(root.c_str(), O_RDONLY | O_DIRECTORY | O_NOFOLLOW | O_CLOEXEC));
  if (anchor.value < 0) return ThrowError(env, "Could not anchor runtime storage.", PosixErrorCode(errno));
  std::vector<int> parents;
  int parent = anchor.value;
  int error = 0;
  for (size_t i = 0; i + 1 < components.size(); ++i) {
    const int next = openat(parent, components[i].c_str(), O_RDONLY | O_DIRECTORY | O_NOFOLLOW | O_CLOEXEC);
  ...
  if (error && error != ENOENT)
    return ThrowError(env, "Could not safely remove runtime tree.", PosixErrorCode(error));
#endif
  napi_value result;
