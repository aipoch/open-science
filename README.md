...
> 💡 **[Open Science v0.18.2 released](https://github.com/aipoch/open-science/releases/latest)** _(last updated August 2026)_. Open Science v0.18.2 adds Russian localization, turn selection for Markdown and PDF conversation exports, and pre-registered OAuth clients for custom connectors, with encrypted secret storage and issuer pinning. Bounded event admission and streamed Project Artifact ZIP exports keep busy workspaces responsive, while notebook runs gain progress heartbeats and UTF-8 source decoding. Attachments now survive message edits, opaque Codex resume failures recover, Task API retries are idempotent, and multiple workspace, update, and persistence issues are fixed. See the [latest release notes](https://github.com/aipoch/open-science/releases/latest) for full details.
