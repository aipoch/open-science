...
> 💡 **[Open Science v0.24.0 released](https://github.com/aipoch/open-science/releases/latest)** _(last updated August 2026)_. Open Science v0.24.0 makes it faster to move between projects and safer to run code: a project quick switcher in the workspace menu, notebook and compute network access limited to the domains you approve, with in-conversation approval for new ones (on Windows, once its administrator setup is complete), device-wide shared credentials that custom connectors can bind to, safe default permissions with one-click restore, and a German interface — alongside immutable file generations for notebook and compute runs, hardened remote compute jobs, and a broad set of runtime, connector, and storage fixes. See the [latest release notes](https://github.com/aipoch/open-science/releases/latest) for full details.
