...
> 💡 **[Open Science v0.21.0 released](https://github.com/aipoch/open-science/releases/latest)** _(last updated August 2026)_. Open Science v0.21.0 adds CodeBuddy as a fourth agent framework with an app-managed, login-free runtime, text and image annotations that send selected context to the agent, greatly expanded OpenCode Go and Zen model catalogs, Windows SSH password authentication for remote compute, agent configuration change markers in the conversation timeline, and a redesigned notification message center and Specialist Marketplace — alongside hardened renderer permissions, connector timeout handling, and a long list of compute, notebook, session, and workspace fixes. See the [latest release notes](https://github.com/aipoch/open-science/releases/latest) for full details.
