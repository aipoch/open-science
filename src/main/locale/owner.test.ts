...
describe('LocalePreferenceOwner', () => {
  it('creates isolated native i18next instances with CLDR plurals and direct English fallback', async () => {
    const messages =
      (await import('./main-process-messages')) as typeof import('./main-process-messages') & {
        createNativeI18n?: (locale: 'en' | 'es' | 'fr' | 'pt-PT' | 'ru' | 'zh-Hans') => {
    ...
    const portuguese = messages.createNativeI18n('pt-PT')
  ...
  it('rejects invalid renderer input and translates native messages with interpolation', async () => {
    ...
    expect(translateNativeMessage('pt-PT', 'Quit', { context: 'verb' })).toBe('Sair')
  ...
  it('detects European Portuguese for main-process surfaces', async () => {
    const owner = new LocalePreferenceOwner(['pt-PT', 'en-US'], await createRepository())

    expect(owner.snapshot()).toEqual({ preference: 'system', locale: 'pt-PT' })
  })
