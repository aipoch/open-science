...
describe('notebook run repository', () => {
  ...
  it('filters root Runs by active Branch before computing the window and summaries', async () => {
    const root = await createStorageRoot()
    const repository = new NotebookRunRepository(root)
    const rootFrameId = 'root-frame-session-1'
    const rootLane = createRootNotebookLane('default-project', 'session-1', rootFrameId)
    const childLane = createFrameNotebookLane('default-project', 'session-1', 'child-frame-1')
    const run = (
      runId: string,
      startedAt: number,
    ...
    for (const rootRun of [
      run('legacy-root', 0, { environment: 'legacy-python' }),
      run('parent-root', 1, {
        rootFrameId,
        agentFrameId: rootFrameId,
        messageBranchId: 'branch-parent',
        environment: 'parent-python'
      }),
      run('active-root', 2, {
        rootFrameId,
        agentFrameId: rootFrameId,
        messageBranchId: 'branch-active',
        environment: 'active-python'
      }),
      run('inactive-root', 4, {
        rootFrameId,
        agentFrameId: rootFrameId,
        messageBranchId: 'branch-inactive',
        environment: 'inactive-python'
      })
    ]) {
      await repository.appendRun({
        projectName: 'default-project',
    ...
    const result = await repository.readSessionRunWindow(
      'default-project',
      'session-1',
      100,
      [],
      rootFrameId,
      { rootFrameId, visibleMessageBranchIds: ['branch-parent', 'branch-active'] }
    )

    expect(result.runs.map((candidate) => candidate.runId)).toEqual([
      'legacy-root',
      'parent-root',
      'active-root',
      'child'
    ...
    expect(result.latestRunEnvironments).toEqual({ python: 'active-python', r: 'child-r' })
    expect(result.historySummary).toEqual({
      agentFrameId: rootFrameId,
      runCount: 3,
      kernelCounts: { python: 3, r: 0, repl: 0, bash: 0 },
      latestDataKernel: 'python'
    })
  })
