...
import { discoverSkillArchive } from '../settings/archive-tasks'
import type { SkillRoot } from './skill-archive-discovery'
...
export class SkillBundleImportOwner {
  ...
  async previewZip(zip: Buffer): Promise<SkillBundlePreviewResult> {
    const { roots, skipped } = await discoverSkillArchive(zip)
    return this.transactions.runRecovered(async () => {
  ...
  async importFromZip(
    ...
    const { roots } = await discoverSkillArchive(zip)
