...
describe('upload command owner', () => {
  ...
  it('resolves a logical Upload preview at read time and preserves an explicit Version', async () => {
    const root = await mkdtemp(join(tmpdir(), 'open-science-upload-preview-owner-'))
    temporaryRoots.push(root)
    const currentPath = join(root, 'current.txt')
    await writeFile(currentPath, 'current upload head')
    ...
    ).resolves.toMatchObject({ content: 'current upload head' })
  ...
  it('reads a logical Upload preview through the verified lease and always closes it', async () => {
    ...
    const owner = createUploadCommandOwner(
      { readManagedUploadPreview } as unknown as UploadRepository,
      { openManagedFileVersion, resolveManagedFilePath }
    )
    const leases = new ApplicationCallerLeaseRegistry()
