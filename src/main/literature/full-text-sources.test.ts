...
describe('additional full-text sources', () => {
  it('uses the Unpaywall contact email only in DOI discovery and separates article and PDF links', async () => {
    ...
    const result = await findUnpaywallPdfs('10.1000/example', 'research@lab.org', fetcher)
  ...
  it('resolves DOI to PMC and discovers version 2 without assuming version 1 exists', async () => {
    ...
    const result = await findPmcPdfs({ doi: '10.1000/example' }, fetcher)
    expect(result).toEqual({
      noRecord: false,
      candidates: [
        {
