...
describe('production application command wiring', () => {
  ...
  it('installs desktop utilities with shared owners and retains find-event cleanup', () => {
    ...
    expect(surface).toContain('const removeFind = registerWindowFindIpcHandlers()')
    expect(surface).toContain('const removeSettingsWindow = registerSettingsWindowIpcHandlers()')
    expect(surface).toContain('removeFind()')
    expect(surface).toContain('removeSettingsWindow()')
