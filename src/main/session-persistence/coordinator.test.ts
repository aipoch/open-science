...
} from '../settings/workflows/runtime'
...
describe('provider removal and saved conversations', () => {
  it.each([
    'preference',
    'claude-code',
    'opencode',
    ...
    'blocks a provider referenced by an archived cross-project %s session without rewriting history',
    async (kind) => {
      const root = await mkdtemp(join(tmpdir(), 'provider-removal-'))
      try {
        const repository = new SessionRepository(root)
        const session = createSession({
          id: 'historical',
        ...
        const providerId = kind === 'legacy Codex' ? 'builtin-codex-subscription' : 'provider-a'
        if (kind === 'preference') {
          session.agentConfiguration = { providerId, model: 'old-model', reasoningEffort: 'high' }
        } else if (kind === 'inactive branch' || kind === 'runtime history') {
          session.agentConfiguration = { providerId: 'replacement', reasoningEffort: 'low' }
          session.conversationGraph = createLinearConversationGraph({
            sessionId: session.id,
            createdAt: 1,
            updatedAt: 2,
            ...(kind === 'runtime history' ? { providerId } : {}),
            messages: [
              {
                ...createUserMessage('historical-message', 1),
          ...
          session.agentBackendId =
            kind === 'legacy Codex' ? 'codex:builtin-codex-isolated' : `${kind}:${providerId}`
        }
        await repository.saveSession(session)
        const path = join(root, 'sessions', 'other-project', 'historical.json')
        const before = await readFile(path, 'utf8')
        const coordinator = new SessionPersistenceCoordinator(repository, createFileIndex())
        const remove = vi.fn().mockResolvedValue({ activeProviderId: 'replacement' })
        const workflow = new RuntimeSettingsWorkflows(
          {
            getSettingsView: async () => ({ activeProviderId: 'replacement' }),
            deleteProvider: remove
          } as unknown as RuntimeSettingsWorkflowStore,
          {
            withProviderRemoval: (ids, operation) =>
              coordinator.withProviderRemoval(ids, operation),
            requestProviderReconnect: vi.fn(),
            requestAgentFrameworkSwitch: vi.fn()
          }
        )
        await expect(workflow.deleteProvider(providerId)).rejects.toThrow(
          'referenced by saved conversations'
        )
        expect(remove).not.toHaveBeenCalled()
        expect(await readFile(path, 'utf8')).toBe(before)
        await expect(
          coordinator.withProviderRemoval(['unreferenced'], async () => 'deleted')
  ...
  it('fails closed on an unreadable session without quarantining or repairing the file', async () => {
    const root = await mkdtemp(join(tmpdir(), 'provider-removal-corrupt-'))
    try {
      const path = join(root, 'sessions', 'other-project', 'broken.json')
      ...
      const remove = vi.fn()
      await expect(coordinator.withProviderRemoval(['provider-a'], remove)).rejects.toThrow(
        'Could not check saved conversations'
      )
      expect(remove).not.toHaveBeenCalled()
  ...
  it('waits for an earlier session save before deciding whether removal is allowed', async () => {
    const root = await mkdtemp(join(tmpdir(), 'provider-removal-order-'))
    try {
      const repository = new SessionRepository(root)
      const coordinator = new SessionPersistenceCoordinator(repository, createFileIndex())
      const saving = coordinator.saveSession(
        createSession({
          agentConfiguration: { providerId: 'provider-a', reasoningEffort: 'high' }
        })
      )
      const remove = vi.fn()
      const removal = coordinator.withProviderRemoval(['provider-a'], remove)
      await saving
      await expect(removal).rejects.toThrow('referenced by saved conversations')
      expect(remove).not.toHaveBeenCalled()
    } finally {
      await rm(root, { recursive: true, force: true })
    }
