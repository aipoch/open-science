...
const removeComputeAnalysisSchema = async (
  ...
  if (computeHostColumns.some(({ name }) => name === 'executionMode')) {
...
describe('application database migrations', () => {
  ...
  it('adds empty metadata commit receipts without inventing proof for historical references', async () => {
    ...
    await client.$executeRawUnsafe(
      `DELETE FROM "_open_science_migrations" WHERE id IN ('0039_literature_metadata_commit_receipt', '0040_literature_collection_revision', '0041_bookmarks', '0042_classification_usage', '0043_pdf_annotations', '0044_literature_smart_collections', '0045_artifact_hidden')`
    )
    ...
    expect(
      await client.$queryRawUnsafe(
        `SELECT * FROM "_open_science_migrations" WHERE id NOT IN ('0039_literature_metadata_commit_receipt', '0040_literature_collection_revision', '0041_bookmarks', '0042_classification_usage', '0043_pdf_annotations', '0044_literature_smart_collections', '0045_artifact_hidden') ORDER BY id`
      )
    ).toEqual(ledger)
  ...
  it('upgrades existing attachment and pending Inbox PDF rows without inventing provenance', async () => {
    ...
    const oldCandidate = literatureCandidateInputSchema.parse({
      item: { itemType: 'journalArticle', title: 'Historical Inbox paper' },
      source: { provider: 'crossref', rawMetadata: {} },
      origin: { kind: 'agent' }
    })
    await client.$executeRawUnsafe(
      `INSERT INTO "ContentBlob" ("id", "storageKey", "checksum", "sizeBytes", "contentType", "state") VALUES ('old-blob', 'content/old', ?, 10, 'application/pdf', 'available')`,
      checksum
    )
    await client.$executeRawUnsafe(
      `INSERT INTO "LiteratureItem" ("id", "itemType", "title", "updatedAt") VALUES ('old-item', 'journalArticle', 'Historical paper', CURRENT_TIMESTAMP)`
    )
    await client.$executeRawUnsafe(
      `INSERT INTO "LiteratureAttachment" ("id", "itemId", "updatedAt") VALUES ('old-attachment', 'old-item', CURRENT_TIMESTAMP)`
    )
    await client.$executeRawUnsafe(
      `INSERT INTO "LiteratureAttachmentVersion" ("id", "attachmentId", "contentBlobId", "versionNumber", "filename", "contentType", "sizeBytes", "checksum", "pageCount") VALUES ('old-version', 'old-attachment', 'old-blob', 1, 'old.pdf', 'application/pdf', 10, ?, 2)`,
      checksum
    )
    await client.$executeRawUnsafe(
      `INSERT INTO "LiteratureInboxCandidate" ("id", "dedupeKey", "itemType", "title", "candidateJson", "metadataChecksum", "origin", "updatedAt") VALUES ('old-candidate', 'old-key', 'journalArticle', 'Historical Inbox paper', ?, ?, 'agent', CURRENT_TIMESTAMP)`,
      JSON.stringify(oldCandidate),
      checksum
    )
    await client.$executeRawUnsafe(
      `INSERT INTO "LiteratureInboxPdf" ("id", "candidateId", "contentBlobId", "filename", "sizeBytes", "checksum", "pageCount", "sourceUrl") VALUES ('old-pdf', 'old-candidate', 'old-blob', 'old.pdf', 10, ?, 2, 'https://example.test/old')`,
      checksum
    )
    await migrateApplicationDatabase(client)
    await migrateApplicationDatabase(client)
    expect(
      await client.literatureAttachmentVersion.findUniqueOrThrow({ where: { id: 'old-version' } })
    ).toMatchObject({ checksum, pageCount: 2, provenanceJson: null })
    expect(
      await client.literatureInboxPdf.findUniqueOrThrow({ where: { id: 'old-pdf' } })
    ).toMatchObject({ sourceUrl: 'https://example.test/old', provenanceJson: null })
    const catalog = new LiteratureCatalog(async () => client!)
    expect((await catalog.get('old-item'))!.attachments[0].versions[0].provenance).toBeUndefined()
    const accepted = await catalog.transact({
      kind: 'accept-candidate',
      candidateId: 'old-candidate'
    })
    expect((await catalog.get(accepted.id))!.attachments[0].versions[0]).toMatchObject({
      filename: 'old.pdf',
      pageCount: 2,
  ...
  it('creates the final literature uniqueness policy and verifies it on restart', async () => {
    ...
    client = createProjectDbClient(storageRoot)
    await migrateApplicationDatabase(client)
    const original = await client.literatureItem.create({
      data: {
        itemType: 'journalArticle',
        title: 'Existing study',
        identifiers: {
          create: { scheme: 'doi', rawValue: '10.1234/study', normalizedValue: '10.1234/study' }
        }
      }
    })
    await expect(
      client.literatureItem.findUnique({ where: { id: original.id } })
    ).resolves.toMatchObject({ title: 'Existing study' })
    await client.literatureItem.create({
      data: {
        itemType: 'journalArticle',
    ...
    await expect(
      client.literatureIdentifier.create({
        data: {
          itemId: original.id,
          scheme: 'doi',
          rawValue: '10.1234/study',
          normalizedValue: '10.1234/study'
        }
      })
    ).rejects.toMatchObject({ code: 'P2002' })
    await expect(migrateApplicationDatabase(client)).resolves.toMatchObject({ applied: [] })
    await client.literatureItem.deleteMany({ where: { id: { not: original.id } } })
    await client.$executeRawUnsafe(
      'CREATE UNIQUE INDEX "LiteratureIdentifier_identity_key" ON "LiteratureIdentifier"("scheme", "normalizedValue")'
    )
  ...
  it('replays 0006 when its constraint indexes are incomplete', async () => {
    ...
    client = createProjectDbClient(storageRoot)
    const notificationIndex = MIGRATION_MANIFEST.findIndex(
  ...
  it('keeps a recovery snapshot when final verification rejects current schema drift', async () => {
    ...
    await expect(
      ...
      code: 'database_validation_failed',
      migrationId: '0045_artifact_hidden'
    })
  ...
  it('repairs preview ownership before resuming an adopted legacy migration', async () => {
    storageRoot = await mkdtemp(join(tmpdir(), 'open-science-database-preview-owner-adoption-'))
    client = createProjectDbClient(storageRoot)
    await createDatabaseAtMigration0005(client)
    await client.$executeRawUnsafe(`INSERT INTO "Project" (
        "id", "name", "updatedAt"
      ) VALUES ('legacy-project', 'Preserved', CURRENT_TIMESTAMP)`)
    await client.$executeRawUnsafe(`INSERT INTO "ProjectPreviewState" (
        "projectId", "panelState", "activeItemId", "items", "updatedAt"
      ) VALUES ('legacy-project', 'open', NULL, '[]', CURRENT_TIMESTAMP)`)
  ...
  it('prunes historical retained backups to the newest two without deleting an unknown backup', async () => {
    ...
    await expect(
      ...
      'open-science.db.before-0045_artifact_hidden.backup',
      unknownBackupName
    ])
  ...
  it('refreshes stale heads and ManagedFile projections while upgrading the pre-managed schema', async () => {
    storageRoot = await mkdtemp(join(tmpdir(), 'open-science-database-current-adoption-'))
    client = createProjectDbClient(storageRoot)
    await createDatabaseAtReleasedManifest(
      client,
      manifestBefore('0025_managed_file_version_foundation')
    )
    ...
    await client.$executeRawUnsafe(`INSERT INTO "ArtifactVersion" (
          ...
          "evidenceJson", "evidenceChecksum", "createdAt", "updatedAt"
        ) VALUES
          (
            'artifact-version-1', 'artifact-1', 1, 'report-v1.md', 'run-1', 'root-1',
            'agent-1', 'branch-1', 'segment-1', 'prompt-1', 'finalized',
            'artifacts/report-v1.md', 'evidence/report-v1.json', 'text/markdown', 10,
            '${'a'.repeat(64)}', '{}', '${'d'.repeat(64)}',
            '2026-08-18T00:00:00.000Z', CURRENT_TIMESTAMP
          ),
          (
            'artifact-version-2', 'artifact-1', 2, 'report-v2.md', 'run-2', 'root-2',
            'agent-2', 'branch-2', 'segment-2', 'prompt-2', 'finalized',
            'artifacts/report-v2.md', 'evidence/report-v2.json', 'text/markdown', 20,
    ...
    await client.$executeRawUnsafe(`INSERT INTO "UploadVersion" (
          "id", "uploadFileId", "versionNumber", "state", "contentStorageKey",
          "filename", "originalFilename", "contentType", "sizeBytes", "checksum", "createdAt",
          "updatedAt"
        ) VALUES (
          'upload-version-1', 'upload-1', 1, 'ready', 'uploads/upload-v1.csv',
  ...
  it('replays an upstream suffix before adopting the canonical managed migration', async () => {
    ...
    client = createProjectDbClient(storageRoot)
    const visionEvidenceIndex = MIGRATION_MANIFEST.findIndex(
      (migration) => migration.id === '0009_vision_evidence'
    )
    expect(visionEvidenceIndex).toBeGreaterThan(0)
    await createDatabaseAtReleasedManifest(client, MIGRATION_MANIFEST.slice(0, visionEvidenceIndex))
