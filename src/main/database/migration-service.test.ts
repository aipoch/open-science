...
describe('application database migrations', () => {
  ...
  it('adds empty metadata commit receipts without inventing proof for historical references', async () => {
    ...
    await client.$executeRawUnsafe(
      `DELETE FROM "_open_science_migrations" WHERE id IN ('0039_literature_metadata_commit_receipt', '0040_literature_collection_revision', '0041_bookmarks', '0042_classification_usage', '0043_pdf_annotations', '0044_artifact_hidden')`
    )
    ...
    expect(
      await client.$queryRawUnsafe(
        `SELECT * FROM "_open_science_migrations" WHERE id NOT IN ('0039_literature_metadata_commit_receipt', '0040_literature_collection_revision', '0041_bookmarks', '0042_classification_usage', '0043_pdf_annotations', '0044_artifact_hidden') ORDER BY id`
      )
    ).toEqual(ledger)
  ...
  it('keeps a recovery snapshot when final verification rejects current schema drift', async () => {
    ...
    await expect(
      ...
      code: 'database_validation_failed',
      migrationId: '0044_artifact_hidden'
    })
  ...
  it('prunes historical retained backups to the newest two without deleting an unknown backup', async () => {
    ...
    await expect(
      ...
      'open-science.db.before-0044_artifact_hidden.backup',
      unknownBackupName
    ])
