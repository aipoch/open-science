...
it('backfills collection revisions without changing existing values or hierarchy', async () => {
  ...
  await client.$executeRawUnsafe(
    "DELETE FROM \"_open_science_migrations\" WHERE id IN ('0040_literature_collection_revision', '0041_bookmarks', '0042_classification_usage', '0043_pdf_annotations', '0044_compute_cancellation_feedback')"
  )
  await expect(
