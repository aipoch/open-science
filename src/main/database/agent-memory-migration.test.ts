...
import { MEMORY_AUXILIARY_SCHEMA_OBJECTS } from './migrations/0013-agent-memory'
...
describe('agent memory migration', () => {
  ...
  it('replays an unledgered partial memory migration and restores missing triggers', async () => {
    ...
    await client.$executeRawUnsafe(
      `DELETE FROM "_open_science_migrations" WHERE "id" = '0013_agent_memory'`
    )

    await expect(migrateApplicationDatabase(client)).resolves.toMatchObject({
      applied: ['0013_agent_memory'],
