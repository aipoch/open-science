...
describe('Compute Job operation migration', () => {
  it('replays an unledgered operation suffix without rewriting ComputeJob', async () => {
    ...
    await client.$executeRawUnsafe(
      `DELETE FROM "_open_science_migrations" WHERE "id" IN ('0023_compute_job_operation', '0024_compute_job_file_evidence', '0025_managed_file_version_foundation', '0026_compute_job_remote_cleanup', '0027_project_session_defaults', '0028_database_numeric_and_null_constraints', '0029_literature_foundation')`
    )
    ...
    ).resolves.toEqual([{ id: '0029_literature_foundation' }])
  })
