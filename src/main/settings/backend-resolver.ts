...
export type AgentBackendRuntimePort = Pick<
  ...
  | 'listLegacyAgentSkillDocumentPaths'
  ...
  private async resolveBackendFromSettings(
    ...
    const codexHome =
      framework.id === 'codex'
        ...
        : undefined
    const disabledCodexSkillPaths = codexHome
      ? await this.runtime.listLegacyAgentSkillDocumentPaths(codexHome)
      : []
    const {
    ...
    const projectedSkillIds = new Set(skillRuntime.descriptors.map((skill) => skill.id))
    const projectedSkillNames = new Set(skillRuntime.descriptors.map((skill) => skill.name))
    const materializedConnectorSkillNames = this.connectors
      .connectorSkillNames(settings.connectors)
      .filter((name) => projectedSkillIds.has(name) || projectedSkillNames.has(name))
    ...
    try {
      ...
      return {
        ...
        env: {
          ...
          ...(framework.id === 'codex'
            ? {
                OPEN_SCIENCE_CODEX_DISABLED_SKILL_PATHS: JSON.stringify(disabledCodexSkillPaths)
              }
            : {})
        },
        skillRuntime,
