...
describe('ConnectorSettingsModule', () => {
  it('blocks bundled enablement for historical custom names without rewriting configuration', async () => {
    ...
    expect(snapshot.connectors.find((connector) => connector.id === 'zenodo')?.enabled).toBe(false)
    expect(snapshot.customServers.find((server) => server.id === 'old-zenodo')?.availability).toBe(
      'unavailable'
    )
    expect(service.enabledConnectorIds(before.connectors)).not.toContain('zenodo')
    expect(service.connectorSkillNames(before.connectors)).not.toContain('mcp-zenodo')
    await expect(service.setConnectorEnabled({ id: 'zenodo', enabled: true })).rejects.toThrow(
      'conflicting ID or name'
    )
    expect(await repository.getSettings()).toEqual(before)
  })

  it('rejects conflicting enablement without migrating historical credential fields', async () => {
    await repository.addCustomServer({
      ...
      command: 'legacy-mcp',
      enabled: false,
      env: { TEST_VALUE: 'synthetic-only' }
    })
    const before = await repository.getSettings()
    const bytes = await readFile(join(dir, 'settings.json'), 'utf8')
    await expect(service.setConnectorEnabled({ id: 'zenodo', enabled: true })).rejects.toThrow(
      'conflicting ID or name'
    )
    expect(await repository.getSettings()).toEqual(before)
    expect(await readFile(join(dir, 'settings.json'), 'utf8')).toBe(bytes)
    expect(keychain.encryptedValues).toEqual([])
  })

  it('keeps bundled identities unavailable after failed deletion cleanup until retry completes', async () => {
    await addHistoricalCustomServer({
      id: 'zenodo',
      name: 'legacy-server',
      transport: 'stdio',
    ...
    await expect(service.removeCustomServer({ id: 'zenodo' }, cleanup)).rejects.toThrow(
      'cleanup failed'
    )
    const pending = (await new SettingsRepository(dir).getSettings()).connectors
    expect(pending?.customMcpServers ?? []).toEqual([])
    expect(pending?.pendingCustomServerDeletionIds).toEqual(['zenodo'])
    expect(service.enabledConnectorIds(pending)).not.toContain('zenodo')
    expect(service.connectorSkillNames(pending)).not.toContain('mcp-zenodo')
    expect(service.connectorSkillCatalogEntries(pending).map((entry) => entry.name)).not.toContain(
      'mcp-zenodo'
    )
    expect(
      (await service.listConnectors()).connectors.find((connector) => connector.id === 'zenodo')
        ?.enabled
    ).toBe(false)
    await expect(service.setConnectorEnabled({ id: 'zenodo', enabled: true })).rejects.toThrow(
      'deletion cleanup is pending'
    )
    const recovered = await service.removeCustomServer({ id: 'zenodo' }, cleanup)
    expect(cleanup).toHaveBeenCalledTimes(2)
    expect(recovered.connectors.find((connector) => connector.id === 'zenodo')?.enabled).toBe(true)
    expect(
      (await repository.getSettings()).connectors?.pendingCustomServerDeletionIds
    ).toBeUndefined()
  })
