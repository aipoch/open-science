...
describe('formatDateTime', () => {
  it('writes the timestamp in the script of the requested locale', () => {
    ...
    const portuguese = formatDateTime(INSTANT, 'pt-PT')
    ...
    expect(portuguese).toMatch(/13\/08/u)
...
describe('formatRelativeTime', () => {
  it('clamps anything under 45 seconds to "now" in the locale language', () => {
    ...
    expect(formatRelativeTime(INSTANT, 'pt-PT', INSTANT)).toMatch(/agora/u)
  ...
  it('picks the coarsest sensible unit in the locale language', () => {
    ...
    expect(formatRelativeTime(INSTANT - 3 * 60_000, 'pt-PT', INSTANT)).toMatch(/há 3 min/u)
