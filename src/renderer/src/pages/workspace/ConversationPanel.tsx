...
const ConversationPanel = ({
  ...
  // Interrupted sessions auto-resume on the next send, so suppress the restore sentinel and only
  // surface the red box for a real error.
  const sessionError =
    activeSession?.error === INTERRUPTED_SESSION_ERROR ? undefined : activeSession?.error
  const displayError = actionError ?? sessionError
          ...
          <div className="px-4 pb-2">
            ...
            <div className={composerContentClassName}>
              <div className="px-1 md:px-3">
                {/* Interrupted sessions look clean and auto-resume on the next send; only a real
                    error surfaces the red box (see displayError above). */}
                {displayError ? (
