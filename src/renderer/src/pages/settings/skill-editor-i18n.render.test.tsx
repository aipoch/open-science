...
const typeId = (value: string): void => {
  const input = container.querySelector<HTMLInputElement>('input.font-mono')
  if (!input) throw new Error('skill id input not found')
  act(() => {
...
describe('SkillEditor copy', () => {
  it('translates the section headings and the create-mode footer verb', () => {
    ...
    switchTo('zh-Hans')
    expect(container.textContent).toContain('标识')
    ...
    expect(container.textContent).toContain('識別資訊')
  })
  ...
  it('re-renders validation messages on a language switch', () => {
    render(blank)
    expect(container.textContent).toContain('Skill ID is required.')

    // The message comes out of a useMemo; without `t` in its deps it would stay English here.
    switchTo('zh-Hans')
    expect(container.textContent).toContain('技能 ID 不能为空。')
    expect(container.textContent).not.toContain('Skill ID is required.')

    typeId('Bad Slug')
    expect(container.textContent).toContain('只能使用小写字母、数字和连字符。')

    typeId('taken')
    expect(container.textContent).toContain('已存在使用该 ID 的技能。')
  })
