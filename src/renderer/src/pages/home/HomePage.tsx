...
type HomeSessionUpdate = {
  ...
  completedNotificationIds: readonly string[]
}
...
const HomePage = ({
  ...
  const markNotificationsRead = useNotificationInboxStore((state) => state.markRead)
  ...
  const unreadCompletedBySession = useMemo(() => {
    const completedBySession = new Map<string, { completedAt: number; notificationIds: string[] }>()

    for (const item of notificationItems) {
      if (item.kind !== 'task.completed' || item.readAt !== undefined || !item.sessionId) continue
      const completed = completedBySession.get(item.sessionId)
      if (completed) {
        completed.completedAt = Math.max(completed.completedAt, item.createdAt)
        completed.notificationIds.push(item.id)
      } else {
        completedBySession.set(item.sessionId, {
        ...
        ? [
            {
              session,
              activity: 'completed',
              activityTimestamp: completed.completedAt,
              completedNotificationIds: completed.notificationIds
            }
          ]
        : []
    })
        ...
        {sessionUpdates.length > 0 ? (
          <section className="mt-8 sm:mt-10" aria-label="Session updates">
            <div className="grid grid-cols-1 gap-3 py-1 md:grid-cols-2">
              {sessionUpdates.map(
                ({ session, activity, activityTimestamp, completedNotificationIds }) => {
                    ...
                    <div key={session.id} className="home-session-card group relative min-w-0">
                      ...
                      {completed ? (
                        <button
                          ...
                          onClick={() =>
                            void dismissCompletedSession(session.id, completedNotificationIds)
                          }
