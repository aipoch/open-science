...
describe('AI review workflow contract', () => {
  ...
  it('runs the Claude agent with zero tools so it cannot read runner secrets', () => {
    // Without Read, Bash, Grep, or Glob the agent cannot access /proc/self/environ or any file.
    // Review context is pre-generated by a trusted step and embedded in the prompt instead.
    expect(reviewWorkflow).toContain('--allowedTools ""')
    const toolGrants = reviewWorkflow.match(/--allowedTools\s+"[^"]*[A-Za-z]/g)
    expect(toolGrants).toBeNull()
