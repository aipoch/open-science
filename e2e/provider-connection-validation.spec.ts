...
test('synchronizes a conversation upstream rejection into the open provider settings', async ({
  app
}, testInfo) => {
  ...
  try {
    ...
    const providerId = await page.evaluate(
      async ({ baseUrl, key }) => {
        ...
        if (!result.providerId) throw new Error('The synthetic runtime provider failed validation.')
    ...
    const current = (await snapshot(page)).providers.find((provider) => provider.id === providerId)!
    expect(current.lastValidationFailure).toMatchObject({ category: 'auth', status: 401 })
