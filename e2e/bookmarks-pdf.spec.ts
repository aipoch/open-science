...
test('restores text and region bookmarks on intrinsically rotated PDF pages', async ({
  ...
  await expect(restoredTextMarker).toHaveAttribute('data-pdf-bookmark-revealed', 'true')
  ...
  await expect(restoredRegionMarker).toHaveAttribute('data-pdf-bookmark-revealed', 'true')
