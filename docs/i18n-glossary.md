...
column records the corresponding independent translation; Korean is recorded independently too.

| en                 | ja           | ko          | zh-Hans | zh-Hant  |
| ------------------ | ------------ | ----------- | ------- | -------- |
| file               | ファイル     | 파일        | 文件    | 檔案     |
| document           | ドキュメント | 문서        | 文档    | 文件     |
| folder             | フォルダー   | 폴더        | 文件夹  | 資料夾   |
| data               | データ       | 데이터      | 数据    | 資料     |
| information        | 情報         | 정보        | 信息    | 資訊     |
| software           | ソフトウェア | 소프트웨어  | 软件    | 軟體     |
| program            | プログラム   | 프로그램    | 程序    | 程式     |
| default            | デフォルト   | 기본값      | 默认    | 預設     |
| settings           | 設定         | 설정        | 设置    | 設定     |
| network            | ネットワーク | 네트워크    | 网络    | 網路     |
| cache              | キャッシュ   | 캐시        | 缓存    | 快取     |
| process            | プロセス     | 프로세스    | 进程    | 行程     |
| thread             | スレッド     | 스레드      | 线程    | 執行緒   |
| queue              | キュー       | 대기열      | 队列    | 佇列     |
| storage            | ストレージ   | 저장소      | 存储    | 儲存     |
| credential         | 認証情報     | 자격 증명   | 凭据    | 憑證     |
| log                | ログ         | 로그        | 日志    | 記錄檔   |
| mirror             | ミラー       | 미러        | 镜像源  | 鏡像來源 |
| tray               | トレイ       | 트레이      | 托盘    | 系統匣   |
| bookmark           | ブックマーク | 북마크      | 书签    | 書籤     |
| archive (verb)     | アーカイブ   | 보관        | 归档    | 封存     |
| approve / approval | 許可 / 承認  | 허용 / 승인 | 批准    | 核准     |
