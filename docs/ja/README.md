...
> 💡 **[AIPOCH Open-Science v0.33.0 をリリースしました](https://github.com/aipoch/open-science/releases/latest)** _（最終更新：2026 年 9 月）_。AIPOCH Open-Science v0.33.0 は、スクリーニング級の文献トリアージをもたらします。スマートな文献コレクションが、採用基準と除外基準に照らして参考文献を評価し、採用 / 要確認 / 除外の状態、PDF エビデンス、オプトインの自動更新を備えます。研究パッケージは RO-Crate メタデータを埋め込むようになりました（新しくエクスポートされたパッケージには ro-crate 対応のリーダーが必要で、既存のパッケージはそのまま読めます）。コネクタも Zenodo の公開レコード検索、GDC のプロジェクトとファイルのメタデータ、UniProt のバッチ識別子マッピングで拡充されます。エージェント単位のリソースアクセス制御により、メインエージェントとスペシャリストのスキルおよびコネクタ権限が 1 か所にまとまり、セッションはローカル診断エクスポートを備え、プロバイダーカタログは Xiaomi MiMo v2.6、Grok 4.7、Zen と Go の現行世代モデルで更新されました。Notebook の環境と回復の通知は閉じられるようになり、Windows のパッケージリセットは読み取り専用属性を解除し、Linux パッケージは Fedora クエリエンジンを同梱し、トークン推定に上限を設けてストリーミングの応答性を保ちます。詳細は[最新リリースノート](https://github.com/aipoch/open-science/releases/latest)を参照してください。
...
[公式ダウンロードページ](https://aipoch.com/open-science/download)を開き、コンピューターに合うインストーラーを選択します。
...
[公式ダウンロードページ](https://aipoch.com/open-science/download)からダウンロードしてください。必要に応じて[ダウンロードの検証](../../SECURITY.md#verifying-your-download)を参照してください。
