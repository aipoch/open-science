...
| 分野                                         | 主な機能                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| -------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **プロジェクトとセッション**                 | プロジェクトを作成・整理し、セッションのピン留め、永続的なメッセージ分岐とサイド会話、編集可能なセッション詳細を利用できます。完了済みプロンプトを、元の後続経路を削除せずに永続的で選択可能なメッセージ分岐として編集し、最近の作業・下書き・会話履歴・プレビュー状態を復元できます。サイド会話は独立したプレビュータブとして開き、操作可能なままのメイン会話と並行して複数を実行し、現在のモデルを引き継ぐ下書きとして開始し、注釈をそれらの間で移動できます。セッションを自己完結型の `.science` 研究パッケージとしてエクスポートし、会話ブランチ、ファイルバージョン、Notebook 記録、検証証拠を伴ったまま、別のプロジェクトやマシンへインポートできます。                                                                                                                                                                                                                                                                                                   |
| **エージェントワークフロー**                 | 自然言語セッションでストリーミング応答と目的別にまとめられたツール活動を確認し、承認・停止、フォローアップのキュー、コンテキスト圧縮、再起動後の復旧を利用できます。完了した作業を新しいセッションへ分岐し、構造化された確認、テキスト・画像・PDF 注釈、リンク済み PDF の読解コンテキスト、プロジェクトメモリ、セッション参照、レビュー必須の計画、そして会話テキスト、プレビュー、PDF の箇所へのメモ付きで、再起動後も保持されるプライベートブックマークを組み合わせられます。カテゴリー別のグローバル検索は、プロジェクト、セッション、メッセージ本文、アップロード済みおよび生成済みファイル、文献ライブラリを対象とし、標準および詳細フィルターと、一致するメッセージへジャンプできる文脈付きの詳細プレビューを備えます。通知、ライブ状態、時間とトークンの詳細、コマンドパレット、ソースプレビュー、プロジェクト切り替えにより、長時間の研究も可視化して管理できます。                                                                                     |
| **モデルとエージェントバックエンド**         | Apodex、厳選されたエージェント対応カタログを持つ NVIDIA Build、最新の OpenAI および Anthropic モデルカタログ（GPT-6 Astra と Claude Fable 5.1）などの組み込みクラウドプロバイダーに加え、互換カスタムゲートウェイ、Claude と Codex のサブスクリプションログインを利用できます。エージェントバックエンドには Claude Code、OpenCode、Codex、またはログイン不要の CodeBuddy を選択でき、モデル/API 互換性の検証、マルチモーダル画像入力、推論制御、専用のサブエージェント・レビュアー・Vision ポリシーを備えます。アプリ管理のランタイムはインプレースで更新され、管理対象の Codex CLI は Settings からのテスト済みアップグレードを提供し、CLI とアダプターのバージョンは別々に表示されます。                                                                                                                                                                                                                                                                      |
...
| **アーティファクトと来歴**                   | チェックサム付き内容、生成コード、実行履歴、正確な入力、環境一覧、メッセージ分岐コンテキスト、系譜、レビュー証拠を含む、セッション単位の不変アーティファクトバージョンを保持します。編集可能な Markdown、テキスト、スクリプト、ソースコードは保存のたびに来歴を維持した新バージョンを公開し、直前の版と比較できます。取得済みの任意のバージョンは、封印されたレシピを隔離環境で再実行して再現されたファイルを元のファイルと比較することで検証できます。バイト完全一致チェック、上限付きの画像と表の比較、オプションの科学的ルール、エクスポート可能な検証記録が利用できます。                                                                                                                                                                                                                                                                                                                                                                                   |
| **科学スキルとデータコネクタ**               | **22 個の注目**組み込みスキルと **24 個の組み込み**研究コネクタで研究ワークフローを拡張できます。会話または完了済み作業からスキルを作成し、パッケージや GitHub ソースを取り込み、ツール単位の権限と設定のインポート/エクスポートを備えたカスタムのローカル/リモート MCP コネクタを追加できます。署名付きカタログを通じて検証済みスキルマーケットプレイスを閲覧できます。カテゴリーと作者でスキルを探し、ワンクリックインストール、バージョン更新、フィルター選択と再試行可能な進捗つきの一括インストールを利用でき、Settings からコネクタを一括管理できます。コアアプリケーションスキルは常に有効なままのため、組み込みの入口が使えなくなることはありません。また、ヘッドレス CLI と Task SDK では、コネクタの一覧表示、確認、有効化/無効化ができます。リソース横断タグ、保護された「お気に入り」タグ、検索可能なフィルターでスキル、コネクタ、スペシャリストを整理します。                                                                                     |
| **ローカルデータ、プライバシー、権限、検証** | プロジェクトデータ、アプリ状態、Notebook キャッシュを設定・移行可能なローカルストレージに保持し、システム・手動・直接接続のプロキシモードと、30 日間のアクティビティヒートマップおよび実行別内訳を備えたトークンダッシュボードを利用できます。`Ask for approval`、`Auto-approve edits`、`Full access`、スコープ付き許可、一元管理された認証情報（ヘッドレス Linux のデプロイでは明示的なファイルストレージモードを選択可能）、ユーザー承認済み計算ドメイン、コネクタ/ツール単位のポリシーで操作を制御します。モデル出力内のリモートメディアは明示的に有効化した後にのみ読み込まれ、モデルへ送信する画像は正規化され、リモートのモデルエンドポイントには HTTPS が必須です。任意のレビュアーは会話記録、実行ログ、アーティファクトを監査して合格/警告/失敗を報告し、永続的な証拠を伴う制限付き修正ループを実行できます。                                                                                                                                          |
