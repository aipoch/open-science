...
Open Science は、チャット、Notebook、ローカルスクリプト、科学データベース、ファイル、レポートツールに分散した研究作業を、実行と根拠が結び付いた永続的でローカルファーストの AI 研究ワークベンチに統合します。
...
[AIPOCH](https://aipoch.com/) は [Open Science](https://aipoch.com/open-science) を、科学 AI ワークフロー向けのオープンソースかつローカルファーストのデスクトップ研究ワークベンチとして開発しています。
...
| 分野                                         | 主な機能                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| -------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **プロジェクトとセッション**                 | プロジェクトを作成・整理し、セッションのピン留め、永続的なメッセージ分岐とサイド会話、編集可能なセッション詳細、最近の作業・下書き・会話履歴・プレビュー状態の復元を利用できます。                                                                                                                                                                                                                                                                                                                                                                                           |
...
| **Python、R、Notebook、HPC**                 | 永続的な Python、R、REPL カーネルと記録されるシェルコマンドを、管理されたオフライン環境または持ち込みのインタープリターで実行できます。ローカルで作業するほか、SSH でリモートホストに接続し、Slurm を通じて HPC クラスターへ Notebook 実行を送信できます。保護されたネットワークアクセス、暗号化された認証情報、パッケージと変数の確認、共有ターミナル、段階的な履歴読み込みにより、計算を制御・観測できます。外部 R ランタイムのパッケージ管理は引き続き手動です。                                                                                                          |
| **文献レビューと参考文献管理**               | DOI、PubMed ID、arXiv ID、またはファイルから参考文献を取り込み、コレクションを整理してプロジェクトへリンクし、ダウンロード済み PDF をゴミ箱から復元できます。Europe PMC、PMC、OpenAlex、arXiv、Unpaywall を並列検索してオープンアクセス全文を取得し、添付ファイルやリンクを失わずに重複レコードを統合し、保存済みメタデータからアーティファクトの来歴付き引用を整形できます。                                                                                                                                                                                                |
...
| **科学スキルとデータコネクタ**               | **22 個の注目**組み込みスキルと **24 個の組み込み**研究コネクタで研究ワークフローを拡張できます。会話または完了済み作業からスキルを作成し、パッケージや GitHub ソースを取り込み、ツール単位の権限と設定のインポート/エクスポートを備えたカスタムのローカル/リモート MCP コネクタを追加できます。リソース横断タグ、保護された「お気に入り」タグ、検索可能なフィルターでスキル、コネクタ、スペシャリストを整理します。                                                                                                                                                         |
| **ローカルデータ、プライバシー、権限、検証** | プロジェクトデータ、アプリ状態、Notebook キャッシュを設定・移行可能なローカルストレージに保持し、システム・手動・直接接続のプロキシモードと、30 日間のアクティビティヒートマップおよび実行別内訳を備えたトークンダッシュボードを利用できます。`Ask for approval`、`Auto-approve edits`、`Full access`、スコープ付き許可、一元管理された認証情報、ユーザー承認済み計算ドメイン、コネクタ/ツール単位のポリシーで操作を制御します。任意のレビュアーは会話記録、実行ログ、アーティファクトを監査して合格/警告/失敗を報告し、永続的な証拠を伴う制限付き修正ループを実行できます。 |
