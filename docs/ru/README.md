...
[![Сайт](https://img.shields.io/badge/Сайт-aipoch.com-2f9e44?style=for-the-badge)](https://aipoch.com/)
...
Open Science — открытая локальная рабочая среда с ИИ для учёных и исследователей, не привязанная к конкретной модели. Она помогает вести воспроизводимые и прозрачные исследования с научными ИИ-агентами, выполнением кода на Python и R, коннекторами научных данных и поддержкой macOS, Windows и Linux. Создайте проект, опишите исследовательскую задачу обычным языком, и агенты смогут читать файлы, искать информацию в интернете, выполнять код, обращаться к научным источникам данных и готовить отчёты, таблицы и графики с прослеживаемым происхождением — всё в одном рабочем пространстве.
...
> 💡 **[Вышла Open Science v0.18.2](https://github.com/aipoch/open-science/releases/latest)** _(последнее обновление: август 2026 г.)_. Open Science v0.18.2 добавляет русскую локализацию, выбор ходов при экспорте беседы и предварительно зарегистрированные OAuth-клиенты для пользовательских коннекторов; также сужены сводки списка агентов, ограничен приём инкрементальных событий, чтобы интерфейс оставался отзывчивым при интенсивных потоках, экспорт ZIP-архивов артефактов проекта выполняется в потоковом режиме, сигналы прогресса MCP сопровождаются декодированием исходного кода ноутбука в UTF-8, вложения сохраняются при редактировании сообщений, восстанавливаются непрозрачные сбои возобновления Codex, создание задач через API идемпотентно, и включён широкий набор исправлений рабочей области, ноутбука, обновлений и сохранности. Полный список изменений приведён в [примечаниях к последнему выпуску](https://github.com/aipoch/open-science/releases/latest).
...
При первом запуске приложение проведёт вас через два этапа:

1. **Среда выполнения агента** — проверка компьютера, выбор и подготовка Claude Code, OpenCode или Codex. Управляемые приложением среды можно установить без Node.js, npm и пароля администратора.
2. **Поставщик моделей** — подключение и проверка нужной модели. Можно выбрать встроенного поставщика, собственный шлюз или существующую подписку Claude либо Codex.

<table>
  <tr>
    <td width="100%"><img src="../images/readme/onboarding-model-provider.jpg" alt="Настройка поставщика модели при первом запуске Open Science"></td>
  </tr>
  <tr>
...
Open Science подготавливает управляемую среду Python в фоновом режиме, но Python не требуется для завершения первоначальной настройки. При запуске Notebook приложение дождётся готовности среды. При самом первом запуске установщика Windows папка больших данных создаётся на диске установки, но не внутри папки приложения. R, собственные интерпретаторы и последующая смена расположения данных остаются в настройках; смена хранилища по-прежнему выполняется через безопасный перенос и перезапуск.
...
- **Выбор моделей.** Используйте встроенного облачного поставщика, совместимый пользовательский шлюз или подписку Claude либо Codex; модель и поддерживаемая ею глубина рассуждений выбираются вместе в поле ввода.
...
| ------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Проекты и сессии**            | Создание, переименование и удаление проектов; несколько закрепляемых сессий; редактирование завершённых запросов в устойчивые, выбираемые ветки сообщений без удаления исходного продолжения; постоянные боковые диалоги внутри сессии; восстановление недавней работы, черновиков, истории диалога и состояния предпросмотра.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
| **Работа агента**               | Задачи на естественном языке; потоковые ответы; типизированные карточки действий инструментов, сгруппированные по объявленной цели; живой индикатор использования контекста с оценкой по категориям; сжатие контекста по запросу; сохранение после перезапуска; остановка и паузы для подтверждения; запоминаемое подтверждение перед закрытием приложения во время задачи; очередь последующих сообщений; ответвление новой сессии от завершённого сообщения агента; системные уведомления с причиной, устойчивые отметки непрочитанных диалогов и привлечение внимания ОС при ожидающем подтверждении; общий центр уведомлений; карточки уточнений по нескольким вопросам; состояние сессий на главной странице; время сообщений и статистика использования; расход токенов по ходам; сведения о среде агента и модели; палитра команд проекта; чтение данных в рамках проекта; действия проекта и контекст агента; улучшенные строки сессий; планы с проверкой рецензентом, устойчивыми контрактами выполнения и командами CLI для просмотра, утверждения и отклонения; плавное отображение ответа; сворачиваемые боковые панели; сочетание клавиш для нового диалога; восстановление сессий после перезапуска приложения. |
| **Делегирование субагентам**    | Рабочее делегирование задач субагентам с устойчивыми сообщениями и восстановлением.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
| **Модели**                      | Встроенные облачные поставщики, совместимые пользовательские шлюзы, вход по подписке Claude и Codex, проверка соединения, мультимодальный ввод изображений для отдельных моделей, единый выбор модели и поддерживаемой глубины рассуждений, быстрое переключение совместимых моделей и поставщиков без переподключения процесса агента, отдельная модель Vision с устойчивой передачей данных изображений для текстовых сред. Доступные поставщики и форматы API проверяются с учётом выбранной среды агента.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| **Среда выполнения агента**     | Выбираемая среда выполнения агента позволяет использовать одно рабочее пространство с разными реализациями агента. Варианты поставщиков и моделей проверяются относительно выбранной среды; управляемые приложением среды можно устанавливать, переключать и удалять в настройках; воспроизведение контекста учитывает путь контекста каждой среды после переключения или возобновления.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
| **Специалисты**                 | Личные профили агентов-специалистов с ограниченным набором возможностей, немедленная передача текущей задачи от главного агента, настройка в диалоге, импорт и экспорт пакетов, неизменяемые идентификаторы вызовов, создаваемые по имени ID с проверяемым переопределением и отдельный маркетплейс специалистов: проверка подписей пакетов, официальные и одобренные пользователем источники GitHub, резервная загрузка через CDN, ход загрузки и разрешение конфликтов навыков. Представления «Установлено» и «Маркетплейс» разделены; проверенный кэшированный каталог открывается сразу и обновляется вручную, специалиста можно установить со страницы сведений, а общие значки возможностей, быстрое редактирование оформления и переходы по строкам возможностей упрощают управление.                                                                                                                                                                                                                                                                                                                                                                                                                                  |
...
| **Распространение и поддержка** | Установщики для macOS, Windows и Linux; упрощённый мастер первоначальной настройки среды, агента, поставщика модели, Notebook и расположения данных; интерфейс на французском, упрощённом и традиционном китайском, японском, корейском и русском языках; README для каждого поддерживаемого языка; дополнительные многоязычные руководства для участников; заметные напоминания и инструкции по обновлению; локальная диагностика; ссылки на сообщество.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
...
| **Подписка Codex**                 | Сначала выберите среду агента Codex, затем в типе поставщика можно выбрать подписку Codex.                                                                                                                                                                                                                                                                   |
...
[AIPOCH](https://aipoch.com/open-science) ([организация на GitHub](https://github.com/aipoch)) развивает Open Science как настольный уровень оркестрации открытых научных процессов с ИИ.
...
О: Завершите два этапа: **«Среда выполнения агента»** и **«Поставщик моделей»**. Исправьте обязательные требования к компьютеру на странице агента, при необходимости установите или восстановите выбранного агента и проверьте соединение с моделью. Python готовится в фоне, а настройка сред и хранилища остаётся в разделе настроек.
...
| [Сайт](https://aipoch.com/)                                              | Обзор продукта, загрузки и остальная экосистема AIPOCH                                     |
