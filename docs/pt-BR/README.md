...
> 💡 **[Open Science v0.20.2 lançado](https://github.com/aipoch/open-science/releases/latest)** _(última atualização: agosto de 2026)_. O Open Science v0.20.2 adiciona uma interface completa em espanhol, visualizações seguras e isoladas no aplicativo para links de fontes nas respostas do agente, um navegador de variáveis ativas para kernels do Notebook em execução, prévias das sessões ao passar o cursor na barra lateral, ações no menu de contexto das abas de visualização e os novos provedores OpenCode Go e OpenCode Zen, além de modelos da família GLM-5.3. A versão também traz renderização mais rápida das conversas, frequência limitada de salvamento para sessões de longa duração e correções na persistência de sessões, na pré-visualização de planos, no arquivamento e nos Notebooks. Consulte as [notas da versão mais recente](https://github.com/aipoch/open-science/releases/latest) para ver todos os detalhes.
