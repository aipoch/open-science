...
- **Manage** opens the breadcrumb-backed **Manage skills** subpage. This focused surface excludes Featured Skills and the ordinary list's group, add, import, edit, export, and per-row switch controls; only Imported and Personal Skills can appear. Source, status, and search filters sit above a separately scrolling list. Full-row labels retain native checkboxes and keyboard focus outlines; selection changes only the checkbox, with no row background or border emphasis. **Select all results** exposes a mixed state and acts on the currently displayed rows. **Selected (N)** shows every selected resource regardless of filters; changing a filter exits selected-only mode while retaining the selection. **Clear selection** also resets selected-only mode.
- The bottom batch dock appears for a selection, pending operation, or result. It contains selected count, **Selected (N)**, **Clear selection**, **Enable selected (N)**, **Disable selected (N)**, and **Delete selected (N)**. Enable/disable remains immediate and reversible. Selection, filters, and duplicate actions are locked while a write runs. Success and partial failure feedback appears in the dock and can be dismissed with **Done**; clearing selection or starting another operation also clears it. Failed and protected resources remain selected.
