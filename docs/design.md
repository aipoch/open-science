...
- The composer input is a `contenteditable` editor (`role="textbox" aria-multiline`), not a textarea, so it can hold inline non-editable mention chips. Its muted `text-text-300` placeholder is `Ask anything — / skills · @ files · # sessions · ⌘K search · ↑↓ history` on macOS and uses `Ctrl+K` on Windows/Linux. `/` selects a Skill, `@` selects an artifact file, and `#` selects an active Session (current Project first, then other active Projects). Session chips persist only `{ type: 'session', sessionId, title }`: the title is a reference-time display snapshot, while navigation and Host reads resolve the globally unique `sessionId` dynamically.
