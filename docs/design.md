...
- Transform motion is limited to tooltips and opted-in information previews, dialogs, sheets, collapsible content, anchored annotation controls, subtle button feedback, the message-center bell on fresh unread arrivals, Smart Collection result transfers, and the tab selection indicators described below, and must respect `motion-reduce`.
- Shared Tooltips and information previews marked `hover-bubble` enter from their Radix-computed anchor over 240ms: scale grows from 0.9 to 1.02 at 60%, then settles to 1, with opacity reaching 1 at the overshoot. This restrained rebound gives hints a spring-like pop without moving their triggers. Only delayed tooltips animate; immediate openings within the shared skip window appear without entry motion. Session and CSL style previews follow the same rule using their existing group warm windows, and keyboard openings remain immediate. Switching between CSL styles preserves the 320px preview width without replaying scale motion. Tooltips and information previews close immediately, with pointer events disabled while closed, so outgoing hints and cards neither overlap the next preview nor retain a Radix dismissable layer that consumes Escape. Owners that unmount content directly (evaluation details) retain immediate dismissal. Reduced motion disables these animations. Preserve information-preview hover delays, pointer leave grace, focus/keyboard behavior, dimensions and collision handling. Click-only Popovers, hover action menus, annotation panels, native `title` hints and moving run-mark previews retain their own presentation.
...
- A new unread item after the first ready snapshot briefly swings the bell; the red dot pops only
  when that arrival changes the unread count from zero to positive. Initial hydration and updates
  to existing items do not animate. Reduced-motion preference disables both animations.
