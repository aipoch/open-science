...
- If a bundled Connector introduced by an upgrade conflicts with a persisted custom Connector ID or name, keep the custom configuration unchanged and unavailable. Suppress the bundled Connector from enabled catalogs, reject its calls and enable attempts with a conflict error, and preserve existing `mcp-<name>` documents in canonical and agent Skill roots. Disabled custom configurations and IDs in the existing pending-deletion journal retain this protection until cleanup completes. Enablement conflict checks read identity metadata without resolving or migrating credentials. Names are compared case-insensitively for filesystem ownership; no automatic rename or migration occurs.
