...
- Agent loading surface is transparent and keeps `px-3 py-2` for stable transcript geometry; elapsed and status text use `text-text-000/70`, while the brand thinking indicator uses `text-text-300`. Show it immediately for a locally submitted active run and whenever the current workspace Agent runtime takes ownership of a foreground prompt without another user message. Tool, status, and permission events keep it visible; the first visible assistant text or image unmounts it, with terminal runtime state as a cleanup fallback.
