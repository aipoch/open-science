...
- Assistant Session Message links show a lazy, no-referrer favicon from `https://<hostname>/favicon.ico`; URL paths on the same hostname share one source so Chromium can coalesce requests and reuse its HTTP cache. Keep the globe fallback on load failure. Valid HTTPS links remain native anchors and use Radix Hover Card to show the locally available source title, hostname, and full URL without preloading the target document or fetching its metadata. Activating an HTTPS link opens its runtime-only source tab directly in the right preview panel; the external-link safety dialog remains for non-HTTPS links. The iframe stays sandboxed and HTTPS-only. For its browser-global registered root frame, the main process upgrades HTTP redirect locations before navigation and removes only response `X-Frame-Options` and CSP `frame-ancestors`; all other response CSP directives, permission denial, and navigation guards remain enforced. This treatment is opt-in from `WorkspaceMessageItem`; Settings, update notes, and file previews keep the plain Agent Markdown link.
