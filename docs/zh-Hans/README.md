...
> 💡 **[Open Science v0.18.2 已发布](https://github.com/aipoch/open-science/releases/latest)** _（最后更新于 2026 年 8 月）_。Open Science v0.18.2 新增俄文本地化、Markdown 和 PDF 对话导出的轮次选择，以及支持加密密钥存储和发行者固定的自定义连接器预注册 OAuth 客户端。受限事件准入和项目产物 ZIP 流式导出可在高负载时保持工作区响应，Notebook 执行则新增进度心跳与 UTF-8 源码解码。消息编辑现会保留附件，不透明的 Codex 恢复失败可自动恢复，Task API 重试具备幂等性，并修复了多项工作区、更新和持久化问题。详情请查看[最新发行说明](https://github.com/aipoch/open-science/releases/latest)。
...
| **模型**         | 内置云服务商、自定义兼容网关、Claude 和 Codex 订阅登录、连接验证、逐模型多模态图像输入、用于模型和模型所支持推理强度的按会话组合输入框选择器、设置中仅用于新会话的默认值，以及带有供纯文本后端使用的持久图像证据中继的专用 Vision 模型选择器。可用服务商和 API 格式会针对所选智能体后端验证。                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
| **智能体后端**   | 可选择智能体框架后端，让同一工作区运行在多种底层智能体实现之上；服务商和模型选择会针对所选后端验证；应用管理的后端可在设置中安装、切换和移除；智能体感知的上下文重放在切换或恢复后遵循各框架的上下文路径。                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| **专家**         | 带限定范围能力的个人专家智能体配置、从主智能体即时进行中的交接、对话式定制、软件包导入/导出、不可变调用标识、基于名称生成且可验证覆盖的 ID，以及限定范围的专家市场；支持签名软件包验证、官方和用户批准的 GitHub 来源、CDN 回退、下载进度和导入时技能冲突解决。市场现已区分“已安装”与“市场”视图，可即时打开已验证缓存列表并手动刷新，支持从详情页直接安装，并提供共享能力图标、外观快速编辑和能力行导航。                                                                                                                                                                                                                                                                                                                                                                                |
| **能力组织**     | 为技能、连接器和可运行专家提供跨资源标签，包含受保护的“收藏夹”标签、分配菜单、徽章、筛选器、可搜索的标签设置浏览器，以及持久化的指针或键盘拖拽排序。                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
| **执行**         | 带持久代码/输出历史的 Python、R 和 REPL 控制平面内核，以及记录在同一运行历史中的无状态命令行命令；用于智能体驱动评估的有界 REPL 推理；支持离线预置的应用管理环境；自带 Python 和 R 解释器；作为额外执行目标的远程 SSH 计算主机，支持密钥或密码认证并使用操作系统加密存储凭据；与智能体共享的用户终端；每个运行时环境的只读已安装软件包清单；以及供智能体侧文件检查的 Notebook 产物读取。外部 R 运行时的软件包管理仍需手动完成。                                                                                                                                                                                                                                                                                                                                                         |
...
| **连接器**       | **24 个内置**研究连接器，具有运行时状态和恢复界面；自定义本地/远程 MCP 连接器使用与可编辑显示名称分离的不可变小写调用名称；提供基于名称生成且可验证覆盖的本地 ID、联系元数据以及连接器/工具级权限。目录交互现采用与技能相同的紧凑管理模式。                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
