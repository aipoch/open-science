...
> 💡 **[Open Science v0.20.1 已发布](https://github.com/aipoch/open-science/releases/latest)** _（最后更新于 2026 年 8 月）_。Open Science v0.20.1 新增生成并可编辑的会话详情（标题与描述）、输入框草稿重做快捷键、以占位符替换凭据的 MCP 客户端配置导入/导出、逐模型调用用量详情与全新按调用划分的上下文窗口图表，以及关联的 HTTP 请求诊断；同时带来与代理设置一致的网络行为、经过校验的下载、更安全的远程访问配对，并修复大量会话、笔记本、连接器、计算与提供商问题。详情请查看[最新发行说明](https://github.com/aipoch/open-science/releases/latest)。
...
| **执行**         | 带持久代码/输出历史的 Python、R 和 REPL 控制平面内核，以及记录在同一运行历史中的无状态命令行命令；用于智能体驱动评估的有界 REPL 推理；支持离线预置的应用管理环境；自带 Python 和 R 解释器；作为额外执行目标的远程 SSH 计算主机，支持密钥或密码认证并使用操作系统加密存储凭据；与智能体共享的用户终端；每个运行时环境的只读已安装软件包清单；供智能体侧文件检查的 Notebook 产物读取；带耗时显示的软件包安装进度；以及长时间运行 Notebook 的渐进式历史加载。外部 R 运行时的软件包管理仍需手动完成。                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
