...
> 💡 **[Open Science v0.21.0 已发布](https://github.com/aipoch/open-science/releases/latest)** _（最后更新于 2026 年 8 月）_。Open Science v0.21.0 新增第四个智能体框架 CodeBuddy（应用管理、无需登录）、可将选中文本与图像作为上下文发送给智能体的批注功能、大幅扩充的 OpenCode Go 与 Zen 模型目录、远程计算的 Windows SSH 密码认证、对话时间线中的智能体配置变更标记，以及重新设计的通知消息中心和专家市场；同时强化渲染器权限与连接器超时处理，并修复大量计算、笔记本、会话与工作区问题。详情请查看[最新发行说明](https://github.com/aipoch/open-science/releases/latest)。
...
| **执行**         | 带持久代码/输出历史的 Python、R 和 REPL 控制平面内核，以及记录在同一运行历史中的无状态命令行命令；用于智能体驱动评估的有界 REPL 推理；支持离线预置的应用管理环境；自带 Python 和 R 解释器；作为额外执行目标的远程 SSH 计算主机，支持密钥或密码认证（含 Windows）并使用操作系统加密存储凭据；与智能体共享的用户终端；每个运行时环境的只读已安装软件包清单；运行中 Python 和 R 内核的只读实时变量浏览器；供智能体侧文件检查的 Notebook 产物读取；带耗时显示的软件包安装进度；以及长时间运行 Notebook 的渐进式历史加载。外部 R 运行时的软件包管理仍需手动完成。                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
