...
| 领域                           | 核心能力                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| ------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **项目与会话**                 | 创建和组织项目，支持会话置顶、持久消息分支和侧边对话以及可编辑会话详情。可将已完成的提示编辑为持久、可选择的消息分支，而不删除原有后续路径，并恢复近期工作、草稿、对话历史和预览状态。侧聊以独立预览标签页打开——可在仍然可交互的主对话旁并行运行多个侧聊，让它们以继承当前模型的草稿启动，并在其间移动批注。可将某个会话导出为自包含的 `.science` 研究包，并导入到其他项目或机器——对话分支、文件版本、Notebook 记录和验证证据都会随包携带。                                                                                                                                                                                                       |
| **智能体工作流**               | 在自然语言会话中获得流式响应和按用途分组的工具活动，并使用审批、停止、排队跟进、上下文压缩及重启恢复。可将已完成工作分支为新会话，并结合结构化澄清、文本/图片/PDF 批注、链接 PDF 阅读上下文、项目记忆、会话引用与评审门控计划，以及跨重启保留的会话文本、预览与 PDF 段落私有书签（可附笔记）。分类全局搜索覆盖项目、会话、消息正文、上传与生成的文件以及文献库，并提供标准与高级筛选及上下文详情预览，可直达匹配的消息。通知、实时状态、耗时与词元详情、命令面板、来源预览和项目切换让长时间研究保持可见、可管理。                                                                                                                                |
| **模型与智能体后端**           | 使用内置云服务商，包括 Apodex、提供精选智能体模型目录的 NVIDIA Build，以及最新的 OpenAI 和 Anthropic 模型目录（GPT-6 Astra 和 Claude Fable 5.1）；也可连接自定义兼容网关，或使用 Claude 和 Codex 订阅登录。可选择 Claude Code、OpenCode、Codex 或无需登录的 CodeBuddy 作为智能体后端，并获得模型和 API 兼容性验证、多模态图片输入、推理强度控制，以及独立的子智能体、审阅者和 Vision 策略。应用管理的运行时支持就地更新——托管的 Codex CLI 可从设置中进行经过测试的升级，并分别显示 CLI 与适配器版本。                                                                                                                                             |
...
| **Python、R、Notebook 与 HPC** | 运行持久化 Python、R 和 REPL 内核以及纳入同一历史记录的命令行指令，可使用托管离线环境或自带解释器。在本机工作，或通过 SSH 连接远程主机并使用 Slurm 向 HPC 集群提交 Notebook 任务；受保护的网络访问、加密凭据、包与变量查看、共享终端和渐进式历史加载让计算过程可控且可观察。长时间的 Notebook、REPL 与 shell 工作可以在后台运行——释放智能体回合的同时保留确切的运行标识、取消能力与溯源，并在本地运行与远程计算作业之间自动交付结果。在 Windows x64 上，可选启用的 WSL2 Bash 预览会将 shell 命令路由到经过就绪检查的 WSL2 环境中。自带 R 运行时可将缺失的软件包安装到经同意批准的个人库，而已捕获的依赖锁定可导出恢复脚本，用于条件式软件包恢复。 |
| **文献综述与参考文献管理**     | 通过 DOI、PubMed ID、arXiv ID 或文件导入参考文献——单个 PDF 通过元数据编辑器导入，或一次导入多个，带逐文件进度、重复处理与重试——并可一目了然地查看当前文献库的参考文献总数；整理合集、将参考文献关联到项目，并从回收站恢复下载的 PDF。并行检索 Europe PMC、PMC、OpenAlex、arXiv 和 Unpaywall 的开放获取全文，通过 DOI 查询出版商元数据和数据集链接，在不丢失附件或链接的情况下合并重复记录，并根据已存元数据生成带产物溯源的引用。在本地从 PDF 中提取图表——重建的图像裁剪、题注与合并表头——复制或导出 HTML、TSV、Markdown 研究表格，或让智能体直接列出并读取已关联 PDF 中提取的图、表与算法。                                                      |
...
| **产物与溯源**                 | 保存不可变、限定于会话的产物版本，包含内容校验和、生成代码、执行历史、确切输入、环境清单、消息分支上下文、沿袭关系和审阅证据。可编辑的 Markdown、文本、脚本和源代码每次保存都会发布保留溯源的新版本，并可与前一版本比较。在隔离环境中重放任一已捕获版本的封存配方，并将复现的文件与原始文件比较，即可验证该版本——涵盖字节级精确检查、有界的图像与表格比对、可选的科学规则，以及可导出的验证记录。                                                                                                                                                                                                                                                 |
| **科学技能与数据连接器**       | 使用 **22 个精选**内置技能和 **24 个内置**研究连接器扩展研究工作流。可通过对话或已完成工作创建技能，导入包和 GitHub 来源，并添加具有工具级权限及配置导入/导出能力的自定义本地或远程 MCP 连接器。通过签名目录浏览经过验证的技能市场——可按类别和作者发现技能，支持一键安装、版本更新，以及带筛选选择与可重试进度的批量安装——并可在设置中批量管理连接器。核心应用技能保持始终启用，内置入口因此持续可用；无头 CLI 与 Task SDK 可以列出、检查以及启用或禁用连接器。跨资源标签、受保护的“收藏夹”标签和可搜索筛选器用于组织技能、连接器和专家。                                                                                                         |
| **本地数据、隐私、权限与验证** | 将项目数据、应用状态和 Notebook 缓存保存在可配置、可迁移的本地存储中；支持系统、手动或直连代理模式，以及包含 30 天活动热图和逐次运行归因的词元仪表板。通过 `Ask for approval`、`Auto-approve edits` 或 `Full access`、限定范围的授权、集中式凭据（无头 Linux 部署可选择显式的文件存储模式）、用户批准的计算域名以及连接器和工具级策略控制操作。模型输出中的远程媒体仅在显式激活后加载，发送给模型的图像会被规范化，且远程模型端点必须使用 HTTPS。可选审阅者会审计对话记录、执行日志和产物，报告通过/警告/失败结果，并可运行保留持久证据的有界修复循环。                                                                                           |
