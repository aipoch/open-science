...
<p align="center">
  ...
  <a href="../pt-BR/README.md"><img alt="巴西葡萄牙语 README" src="https://img.shields.io/badge/Português%20(Brasil)-d9d9d9"></a>
...
> 💡 **[Open Science v0.20.2 已发布](https://github.com/aipoch/open-science/releases/latest)** _（最后更新于 2026 年 8 月）_。Open Science v0.20.2 新增完整的西班牙语界面、在应用内安全预览智能体回复中的来源链接、针对运行中笔记本内核的实时变量浏览器、侧边栏会话悬停预览、预览标签页右键菜单，以及 OpenCode Go 与 Zen 服务商和 GLM-5.3 系列模型；同时加快对话渲染、限制长会话的保存频率，并修复会话持久化、计划预览、归档和笔记本等多方面问题。详情请查看[最新发行说明](https://github.com/aipoch/open-science/releases/latest)。
...
| **专家**         | 带限定范围能力的个人专家智能体配置、从主智能体即时进行中的交接、对话式定制、软件包导入/导出、不可变调用标识、基于名称生成且可验证覆盖的 ID，以及限定范围的专家市场；支持签名软件包验证、官方和用户批准的 GitHub 来源、CDN 回退、下载进度和导入时技能冲突解决。市场现已区分“已安装”与“市场”视图（单一主浏览入口与明确返回路径），可即时打开已验证缓存列表并手动刷新，支持从详情页直接安装，并提供共享能力图标、外观快速编辑和能力行导航。                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| **能力组织**     | 为技能、连接器和可运行专家提供跨资源标签，包含受保护的“收藏夹”标签、分配菜单、徽章、筛选器、可搜索的标签设置浏览器，以及持久化的指针或键盘拖拽排序。                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| **执行**         | 带持久代码/输出历史的 Python、R 和 REPL 控制平面内核，以及记录在同一运行历史中的无状态命令行命令；用于智能体驱动评估的有界 REPL 推理；支持离线预置的应用管理环境；自带 Python 和 R 解释器；作为额外执行目标的远程 SSH 计算主机，支持密钥或密码认证并使用操作系统加密存储凭据；与智能体共享的用户终端；每个运行时环境的只读已安装软件包清单；运行中 Python 和 R 内核的只读实时变量浏览器；供智能体侧文件检查的 Notebook 产物读取；带耗时显示的软件包安装进度；以及长时间运行 Notebook 的渐进式历史加载。外部 R 运行时的软件包管理仍需手动完成。                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
...
| **分发与支持**   | macOS、Windows 和 Linux 安装程序；针对环境、智能体运行时、模型服务商、Notebook 运行时和数据位置的精简首次启动向导；支持西班牙语、法语、简体中文、繁体中文、日语、韩语、巴西葡萄牙语和俄语界面，为每种受支持语言提供 README 译本，并另有多语言贡献指南；带醒目更新提醒的更新指引；本地诊断；社区链接。                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
...
当前内置云厂商包括 OpenAI、Anthropic、Grok (xAI)、DeepSeek、带专用 GLM Coding Plan 端点的智谱 AI (GLM)、Kimi (Moonshot)、MiniMax、带专用 Step Plan 订阅端点的 StepFun、小米 MIMO、SenseNova、Volcengine Ark、带专用 Bailian for Plan 订阅端点的百炼 (Alibaba Cloud)，以及 OpenCode Go、OpenCode Zen 和 OpenRouter 聚合网关等；部分具有地区限制。
