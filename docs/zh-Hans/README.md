...
Open Science 包含不断扩展的 **22 个精选**文件型研究技能目录：AlphaFold2、Boltz、Borzoi、Chai-1、Customize、DiffDock、Environment & Packages、ESM-2、ESMFold2、Evo 2、Figure Composer、Figure Style、Indication Dossier、LigandMPNN、Literature Review、OpenFold3、Paper Narrative、ProteinMPNN、scGPT、scvi-tools、SolubleMPNN，以及用于在远程 HPC 集群提交和收取长时间任务的 **Remote Compute (SSH)**。你可以新建个人技能，上传 `SKILL.md`/ZIP/`.skill` 软件包，以可选认证方式从 GitHub 预览并导入兼容技能，或导入已安装在全局智能体目录中的技能。智能体也可以请求从会话附件或公开 GitHub URL 导入软件包；应用会在写入任何内容前提供自有的预览和确认步骤。可以在输入框中使用 `/` 直接选择已启用的技能。
...
<p align="center">
  <img src="../images/readme/biomnibench-public50-leaderboard.jpg" alt="BiomniBench-DA Public 50 对比，其中 AIPOCH Open Science 以 79.05 分排名第一" width="1200" />
</p>
...
| **智能体工作流** | 自然语言任务、流式响应、按声明目的分组的类型化工具活动卡片、消息与批准中用于产物写入和 Notebook 控件的紧凑摘要卡片、带分类估算的实时上下文用量指示器、按需上下文压缩、跨重启持久化、停止控件、批准暂停、运行任务期间关闭或退出前的确认步骤（可记住偏好）、运行轮次期间用于暂存后续消息的输入框消息队列、统一的草稿撤销与重做历史、从已完成智能体消息分支到新会话、带关注原因的桌面通知以及持久未读对话徽章和阻塞批准的原生提醒、带类型与状态编码图标、持久读取状态和已删除目标保留的跨界面通知消息中心、处理多问题请求的结构化智能体澄清卡片（支持逐题答案回顾）、可将选定上下文发送到对话的文本、图像与 PDF 批注（选中的文本或区域在源文档中点击即可定位）、标记轮次之间智能体配置变更的时间线标记、展开时显示所加载技能文档的技能加载行、可将最多三个 PDF 关联为会话阅读上下文（让智能体阅读当前页、逐页浏览完整文档并跨文档检索）、跨会话召回项目范围知识的持久智能体记忆、智能体回复中来源链接的应用内沙箱预览、主页仪表板中的实时会话状态、带耗时和用量弹出框的消息时间元数据、逐轮词元用量（含逐模型调用用量详情与按调用划分的上下文窗口图表）、已完成轮次的智能体框架和模型标识、项目级命令面板、项目级框架读取、项目操作和智能体上下文、工作区项目菜单中列出其他活动项目并带标题与描述预览的项目快速切换器（列表增多后提供模糊搜索）、带会话标题与描述悬停预览的优化会话侧栏行、输入框中引用其他会话的会话引用（`#`，按轮次授予只读访问）、注入正在运行主回合的侧边对话建议、全局搜索的会话编号查找、带持久执行契约以及 CLI 计划查看、批准和拒绝命令的审查门控会话计划、平滑实时响应渲染、可折叠侧面板、新建对话键盘快捷键，以及应用重启后恢复中断会话。 |
| **子智能体委派** | 生产级子智能体委派，支持持久消息和恢复；输入框的智能体控制中还有一个按会话生效的委派开关，决定智能体是否可以把工作委派出去。                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
| **模型**         | 内置云服务商（包括 Apodex、带精选具备智能体能力目录的 NVIDIA Build，以及涵盖 GPT-6 Astra 和 Claude Fable 5.1 的最新 OpenAI 与 Anthropic 模型目录）、自定义兼容网关、Claude 和 Codex 订阅登录、连接验证、逐模型多模态图像输入、用于模型和模型所支持推理强度的按会话组合输入框选择器、涵盖子智能体、审查者和视觉策略的合并场景模型卡片、设置中用于新会话的默认值，以及带有供纯文本后端使用的持久图像证据中继的专用 Vision 模型选择器。可用服务商和 API 格式会针对所选智能体后端验证。 |
...
| **执行**         | 带持久代码/输出历史的 Python、R 和 REPL 控制平面内核，以及记录在同一运行历史中的无状态命令行命令；Notebook 与计算运行时的出站网络访问仅限 Open Science 默认域与用户批准的域，被阻止的目标会在对话中呈现以供批准，保护状态会在 Settings → Runtimes 中显示；用于智能体驱动评估的有界 REPL 推理；支持离线预置的应用管理环境，可在设置中安全重装受管运行时，并提供决定智能体是否可以创建运行时环境的全局开关；自带 Python 和 R 解释器；作为额外执行目标的远程计算主机，支持密钥或密码认证（含 Windows）并使用操作系统加密存储凭据，每台主机都有独立的执行模式——直连 SSH 或 Slurm——因此 Notebook 运行可以在按 HPC 分区的集群上作为 Slurm 作业提交，并配有引导式设置技能，把集群的确切设置、修复和移除步骤交给你或管理员；与智能体共享的用户终端，输入时会从运行中的内核实时建议变量名；每个运行时环境的只读已安装软件包清单；运行中 Python 和 R 内核的只读实时变量浏览器；供智能体侧文件检查的 Notebook 产物读取；会话活动中带耗时显示的软件包安装进度；以及长时间运行 Notebook 的渐进式历史加载。外部 R 运行时的软件包管理仍需手动完成。 |
...
| **文献**         | 开箱即用的项目级文献库：按标识符（DOI、PubMed ID、arXiv ID 等）或从文件导入参考文献，整理进合集并关联到项目，下载的 PDF 会经由收件箱流转，并支持从回收站恢复；通过公开的开放获取来源（Europe PMC、PMC、OpenAlex 和 Unpaywall）查找并附加全文 PDF，可选用凭据；并排比较重复题录并批量合并，同时保留附件与关联；以及基于已存储的元数据格式化引用并附带产物来源。 |
| **产物与来源**   | 不可变、限定到会话的产物版本，包含内容校验和以及可用的生成代码、执行历史、准确输入引用、环境清单、生成消息分支上下文、产物沿袭访问和限定到版本的审查证据；支持版本导航和相关证据间的直接链接；列入白名单的文本产物与上传内容（Markdown、纯文本、脚本和源代码）可以按原始文本编辑，每次保存都会发布一个保留其来源沿袭的新版本，并通过 Compare 操作查看前一个版本。                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
| **预览格式**     | 对常见科学数据、支持文本选择、区域选择、大纲与缩略图导航、文档搜索和页面导航的 PDF、Office 文档（DOCX、XLSX、PPTX）、图像（包括 TIFF，支持缩放和平移）、带语法高亮的源代码、分子结构与反应以及 Notebook 历史提供响应式多标签页预览；支持内联或全屏查看，提供右键标签页操作（关闭、关闭其他、下载、复制路径、另存为产物）、右键内容菜单（复制路径、下载或将本地文件另存为产物，以及检查受管产物的来源或返回其原始上下文），并可通过“在上下文中查看”导航回生成产物的对话。 |
...
| **技能**         | **22 个精选**内置技能；名称不可变且使用小写连字符的个人技能；在会话中通过自然语言对话新建技能；从已完成对话轮次另存为技能；直接用户技能文件夹支持及带外软件包验证；按来源、状态和文本筛选的批量启用/停用管理；软件包上传，支持包含大量资源文件的大型软件包；经认证的 GitHub 预览/导入；预览候选后导入已安装的全局技能；由智能体请求从会话附件或 GitHub URL 导入软件包；用于技能脚本且带结构化验证的 camelCase Host JavaScript API；带注册图形样式、多面板组合与论文叙事辅助工具的来源感知图形工作流；启用/停用控件；以及在会话中通过 `/` 明确选择。重新设计的技能面板统一了主智能体与专家筛选，使用真实用户头像堆叠与有界“使用者”弹窗，整合行操作，并支持经确认的批量删除，同时保护内置及专家关联技能。 |
...
| **安全控制**     | `Ask for approval`、`Auto-approve edits` 和 `Full access` 对话配置；包含代码预览以及调用/对话决定的批准对话框；在当前回合持续生效的权限拒绝，阻止智能体通过其他路径重试或变相执行被拒绝的操作；持久化的全局、项目和会话级允许授权，支持筛选、逐行和按系列撤销以及撤销操作；针对 GitHub 令牌、连接器密钥与连接器登录的集中凭据管理，提供健康状态与引导式恢复；可供自定义连接器绑定而非自行存储副本的设备级共享凭据（API 密钥、访问令牌与 OAuth 登录）；覆盖常规只读检查（Notebook 运行时及其状态、记忆查询和软件包清单）与已批准计划进度更新的安全默认授权，让日常工作少被打断；可重新补齐缺失安全默认授权的恢复默认值操作；另有逐连接器和逐工具策略。 |
| **审查与验证**   | 可选择启用的审查器根据已完成轮次自身的对话记录、执行日志和产物进行审计，报告通过/警告/失败结果，并可运行有界修复循环进行纠正；可配置审查模型策略，可跟随活动模型或固定专用服务商、模型和推理强度；持久化审查评估快照保留纠正归因。                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| **分发与支持**   | macOS、Windows 和 Linux 安装程序；针对环境、数据位置、智能体运行时、模型服务商和 Notebook 运行时的精简首次启动向导；支持德语、西班牙语、法语、简体中文、繁体中文、日语、韩语和俄语界面，为每种受支持语言提供 README 译本，并另有多语言贡献指南；带醒目更新提醒的更新指引；无界面 Task API 与 CLI，带会话配置控件，可在不提交提示的情况下检查和更新会话的模型、记忆与计算主机设置；本地诊断；以及社区链接。 |
