...
<p align="center">
  ...
  <a href="../pt-BR/README.md"><img alt="README en portugais brésilien" src="https://img.shields.io/badge/Português%20(Brasil)-d9d9d9"></a>
...
> 💡 **[Open Science v0.20.1 est disponible](https://github.com/aipoch/open-science/releases/latest)** _(dernière mise à jour : août 2026)_. Open Science v0.20.1 ajoute des détails de session générés et éditables (titre et description), un raccourci de rétablissement de brouillon dans le composeur, l'import/export de configurations client MCP avec remplacement des identifiants par des espaces réservés, des détails d'usage par appel de modèle avec un nouveau graphique de fenêtre de contexte par appel, et des diagnostics de requêtes HTTP corrélés ; en parallèle, un réseau cohérent avec les paramètres de proxy, des téléchargements validés, un appariement d'accès distant renforcé et une longue liste de correctifs de sessions, notebooks, connecteurs, calcul et fournisseurs. Consultez les [notes de version les plus récentes](https://github.com/aipoch/open-science/releases/latest) pour tous les détails.
...
| **Exécution**                  | Noyaux persistants Python, R et REPL de plan de contrôle avec historique durable du code et des sorties, plus des commandes de terminal sans état enregistrées dans le même historique d'exécution ; inférence REPL bornée pour l'évaluation menée par l'agent ; environnements gérés par l'application avec provisionnement hors ligne ; interpréteurs Python et R apportés par l'utilisateur ; hôtes de calcul SSH distants avec authentification par clé ou mot de passe et stockage des identifiants chiffré par le système d'exploitation comme cibles d'exécution supplémentaires ; un terminal utilisateur partagé avec l'agent ; un inventaire en lecture seule des paquets installés par environnement d'exécution ; un accès en lecture aux artefacts Notebook pour l'inspection de fichiers côté agent ; la progression d'installation des paquets avec durée écoulée ; et le chargement progressif de l'historique pour les notebooks à exécution longue. La gestion des paquets pour les environnements d'exécution R externes reste manuelle.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
...
| **Distribution et support**    | Installateurs pour macOS, Windows et Linux ; assistant de premier lancement allégé pour l'environnement, l'environnement d'exécution de l'agent, le fournisseur de modèle, l'environnement d'exécution Notebook et l'emplacement des données ; interfaces en espagnol, français, chinois simplifié et traditionnel, japonais, coréen, portugais brésilien et russe, avec un README dans chaque langue prise en charge et d'autres guides de contribution multilingues ; guidage de mise à jour avec rappels visibles ; diagnostics locaux ; et liens communautaires.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
