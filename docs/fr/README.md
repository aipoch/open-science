...
> 💡 **[AIPOCH Open-Science v0.33.0 est disponible](https://github.com/aipoch/open-science/releases/latest)** _(dernière mise à jour : septembre 2026)_. AIPOCH Open-Science v0.33.0 apporte un tri de littérature digne d’un criblage systématique : les collections de littérature intelligentes évaluent les références selon vos critères d’inclusion et d’exclusion, avec les états Inclus / À examiner / Exclu, les preuves PDF et les mises à jour automatiques sur activation. Les paquets de recherche intègrent désormais les métadonnées RO-Crate (les paquets nouvellement exportés exigent un lecteur prenant en charge ro-crate ; les paquets existants restent lisibles), et la famille des connecteurs s’élargit avec la découverte d’enregistrements publics Zenodo, les métadonnées de projets et de fichiers GDC et le mappage par lots d’identifiants UniProt. Les contrôles d’accès aux ressources par agent réunissent au même endroit les autorisations de compétences et de connecteurs de l’agent principal et des spécialistes, les sessions gagnent un export local des diagnostics, et les catalogues de fournisseurs se rafraîchissent avec Xiaomi MiMo v2.6, Grok 4.7 et des modèles de dernière génération chez Zen et Go. Les avis d’environnement et de récupération du Notebook deviennent fermables, la réinitialisation des paquets Windows efface les attributs en lecture seule, les paquets Linux embarquent le moteur de requêtes Fedora, et la diffusion en continu reste réactive grâce à une estimation de jetons bornée. Consultez les [notes de version les plus récentes](https://github.com/aipoch/open-science/releases/latest) pour tous les détails.
