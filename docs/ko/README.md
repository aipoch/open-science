...
Homebrew는 Apple Silicon 또는 Intel용 패키지를 자동으로 선택합니다. 업데이트, 제거 및 수동 설치에서 전환하는 방법은 [설치 가이드(영문)](../installation.md)를 참고하세요.
...
| **에이전트 워크플로** | 자연어 작업, 스트리밍 응답, 선언된 목적별로 묶인 형식화 도구 활동 카드, 메시지와 승인에서 아티팩트 쓰기와 Notebook 컨트롤을 표시하는 간결한 요약 카드, 범주별 추정이 있는 실시간 컨텍스트 사용량 표시, 요청 시 컨텍스트 압축과 재시작 후 상태 보존, 중지 컨트롤과 승인 일시 정지, 실행 중 작업을 닫거나 종료하기 전의 확인 단계(설정 저장), 실행 중 턴 동안 후속 메시지를 대기시키는 작성창 메시지 대기열, 통합된 초안 실행 취소·다시 실행 기록, 완료된 에이전트 메시지에서 새 세션으로 분기, 주의 사유가 있는 데스크톱 알림과 영구 미확인 대화 배지, 차단 승인에 대한 기본 알림, 종류·상태별 아이콘과 영구 읽음 상태, 삭제된 대상 보존을 갖춘 교차 화면 알림 메시지 센터, 여러 질문 요청에 대한 질문별 답변 검토가 있는 구조화된 에이전트 확인 카드, 선택한 컨텍스트를 대화로 보내는 텍스트·이미지·PDF 주석(선택한 텍스트나 영역은 출처 문서에서 클릭해 바로 확인), 턴 사이 에이전트 구성 변경을 표시하는 타임라인 마커, 펼쳤을 때 로드된 스킬 문서를 보여주는 확장된 스킬 로드 행, 에이전트가 현재 페이지를 읽고 전체 문서를 훑고 검색할 수 있도록 최대 세 개의 PDF를 연결하는 세션 읽기 컨텍스트, 세션을 넘어 프로젝트 범위 지식을 불러오는 영구 에이전트 메모리, 에이전트 응답 내 소스 링크의 앱 내 샌드박스 미리보기, 홈 대시보드의 실시간 세션 상태, 경과 시간과 사용량 팝오버가 있는 메시지 타이밍 메타데이터, 모델 호출별 사용량 세부 정보와 호출별 컨텍스트 창 차트가 있는 턴별 토큰 사용량, 완료된 턴의 에이전트 프레임워크와 모델 식별, 프로젝트 범위 명령 팔레트, 프로젝트 범위 프레임 읽기, 프로젝트 작업과 에이전트 컨텍스트, 목록이 늘어나면 제목·설명 미리보기와 퍼지 검색으로 다른 활성 프로젝트를 나열하는 워크스페이스 프로젝트 메뉴의 프로젝트 빠른 전환기, 세션 제목과 설명의 호버 미리보기를 갖춘 개선된 세션 사이드바 행, 턴 범위 읽기 접근이 있는 작성창의 다른 세션 참조(`#`), 실행 중인 메인 턴에 주입되는 사이드 채팅 조언, 전역 검색의 세션 번호 조회, 영구 실행 계약과 CLI 계획 보기·승인·거부 명령이 있는 리뷰 게이트 세션 계획, 매끄러운 실시간 응답 렌더링, 접을 수 있는 사이드 패널, 새 대화 키보드 단축키, 애플리케이션 재시작으로 중단된 세션 복구. |
| **서브에이전트 위임** | 영구 메시지와 복구를 갖춘 프로덕션 서브에이전트 위임, 작성창 에이전트 컨트롤에서 에이전트가 작업을 위임할 수 있는지 결정하는 세션별 위임 스위치.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| **모델**              | Apodex, 에이전트 지원 카탈로그를 엄선한 NVIDIA Build, 최신 OpenAI 및 Anthropic 모델 카탈로그(GPT-6 Astra와 Claude Fable 5.1)를 포함한 기본 클라우드 제공업체, 사용자 지정 호환 게이트웨이, Claude 및 Codex 구독 로그인, 연결 검증, 모델별 멀티모달 이미지 입력, 모델과 지원 추론 강도를 하나로 합친 세션별 작성창 선택기, 서브에이전트·리뷰어·비전 정책을 아우르는 통합 시나리오 모델 카드, 새 세션의 설정 기본값, 텍스트 전용 백엔드를 위한 영구 이미지 증거 릴레이가 있는 전용 Vision 모델 선택기. 사용 가능한 제공업체와 API 형식은 선택한 에이전트 백엔드에 맞춰 검증됩니다.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
...
| **실행**              | 영구 코드/출력 기록이 있는 Python·R·REPL 제어 영역 커널, 같은 실행 기록에 저장되는 무상태 셸 명령, Open Science 기본값과 사용자가 승인한 도메인으로 제한되고 차단된 대상은 대화 내 승인으로 안내되며 보호 상태는 Settings → Runtimes에서 확인할 수 있는 Notebook 및 컴퓨팅 런타임의 아웃바운드 네트워크 액세스, 에이전트 평가용 제한된 REPL 추론, 오프라인 프로비저닝과 Settings에서의 안전한 관리 런타임 재설치, 에이전트가 런타임 환경을 만들 수 있는지에 대한 전역 토글을 갖춘 앱 관리 환경, 사용자가 직접 준비하는 Python·R 인터프리터, 키 또는 비밀번호 인증(Windows 포함)과 OS 암호화 자격 증명 저장을 갖추고 호스트별 실행 모드(직접 SSH 또는 Slurm)에 따라 Notebook 실행이 HPC 파티션 클러스터에 Slurm 작업으로 제출될 수 있으며 클러스터의 정확한 설정·복구·제거 단계를 사용자나 관리자에게 전달하는 안내식 설정 스킬이 제공되는 추가 실행 대상인 원격 컴퓨팅 호스트, 실행 중인 커널의 실시간 변수 이름 제안을 입력하면서 보여주는 에이전트와 공유하는 사용자 터미널, 런타임 환경별 읽기 전용 설치 패키지 목록, 실행 중인 Python·R 커널을 위한 읽기 전용 실시간 변수 브라우저, 에이전트 측 파일 검사를 위한 Notebook 아티팩트 읽기, 세션 활동에 경과 시간이 표시되는 패키지 설치 진행률, 장기 실행 노트북의 점진적 기록 로딩. 외부 R 런타임의 패키지 관리는 수동으로 유지됩니다.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
...
| **문헌**              | 프로젝트에 바로 쓸 수 있는 참고문헌 라이브러리. 식별자(DOI, PubMed ID, arXiv ID 등)나 파일로 참고문헌을 가져오고, 컬렉션으로 정리하고, 프로젝트에 연결하고, 다운로드한 PDF는 휴지통 복구가 있는 받은 편지함으로 전달합니다. 자격 증명을 선택적으로 사용해 공개 오픈 액세스 소스(Europe PMC, PMC, OpenAlex, arXiv, Unpaywall)에서 적용 가능한 모든 소스를 병렬로 조회하여 전문 PDF를 찾아 첨부합니다. 중복 문헌을 나란히 비교하고 첨부 파일과 연결을 유지한 채 일괄 병합합니다. 그리고 저장된 메타데이터에서 아티팩트 출처와 함께 인용 형식을 지정합니다.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
...
| **미리보기 형식**     | 일반 과학 데이터, 선택 가능한 텍스트·영역 선택·목차 및 썸네일 탐색·문서 검색·페이지 탐색이 있는 PDF, Office 문서(DOCX, XLSX, PPTX), TIFF를 포함한 확대/축소 및 이동 가능 이미지, 구문 강조 소스 코드, 분자 구조와 반응, Notebook 기록의 반응형 다중 탭 미리보기. 인라인 또는 전체 화면으로 볼 수 있고, 탭 마우스 오른쪽 버튼 작업(닫기, 다른 탭 닫기, 다운로드, 경로 복사, 아티팩트로 저장), 경로 복사·다운로드·로컬 파일을 아티팩트로 저장하고 관리 아티팩트의 출처를 살펴보거나 원래 컨텍스트로 돌아가는 콘텐츠 마우스 오른쪽 버튼 메뉴, 그리고 아티팩트를 만든 대화로 돌아가는 컨텍스트에서 보기 탐색을 지원합니다.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
...
| **스킬**              | **22개의 주요** 기본 스킬, 불변 소문자 하이픈 이름을 갖는 개인 스킬, 세션 안에서 자연어로 스킬을 만드는 대화식 생성, 완료된 대화 턴에서 스킬로 저장, 외부 패키지 검증이 포함된 직접 사용자 스킬 폴더 지원, 소스·상태·텍스트 필터가 있는 일괄 활성화/비활성화 관리, 대용량·에셋 중심 번들을 포함한 패키지 업로드, 인증된 GitHub 미리보기/가져오기, 후보 미리보기가 있는 설치된 전역 스킬 가져오기, 세션 첨부 파일이나 GitHub URL에서 에이전트가 요청하는 패키지 가져오기, 구조화 검증이 있는 스킬 스크립팅용 camelCase Host JavaScript API, 등록된 스타일링·구성·논문 내러티브 헬퍼를 갖춘 출처 인식 그림 워크플로, 활성화/비활성화 컨트롤, 세션에서 `/`로 명시적으로 선택. 재설계된 스킬 패널은 메인 에이전트와 스페셜리스트 필터를 통합하고, 실제 사용자 아바타 스택과 제한된 '사용 중' 팝오버를 표시하며, 행 작업을 통합하고, 기본 및 스페셜리스트 연결 스킬을 보호하면서 확인된 일괄 삭제를 지원합니다.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
...
| **안전 제어**         | `Ask for approval`, `Auto-approve edits`, `Full access` 대화 프로필, 코드 미리보기와 호출/대화 결정이 있는 승인 대화상자, 거부된 작업의 재시도나 우회를 막는 턴 범위 거부, 필터·행별/계열별 취소·실행 취소가 있는 영구 전역·프로젝트·세션 범위 허용 권한, 상태 표시와 안내 기반 복구가 있는 GitHub 토큰·커넥터 키·커넥터 로그인의 통합 자격 증명 관리, 사용자 지정 커넥터가 자체 복사본을 저장하는 대신 바인딩할 수 있는 기기 전역 공유 자격 증명(API Key, 액세스 토큰, OAuth 로그인), 루틴 읽기 전용 검사(Notebook 런타임과 상태, 메모리 조회, 패키지 목록)와 승인된 계획 진행 업데이트를 다루어 일상 작업의 중단을 줄이는 안전 기본 권한 부여, 누락된 안전 기본 권한 부여를 다시 추가하는 기본값 복원 작업, 그리고 커넥터별·도구별 정책.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
...
| **배포 및 지원**      | macOS, Windows, Linux 설치 프로그램, 환경·데이터 위치·에이전트 런타임·모델 제공업체·Notebook 런타임을 위한 간결한 최초 실행 온보딩 마법사, 독일어·스페인어·프랑스어·중국어 간체·중국어 번체·일본어·한국어·러시아어 인터페이스 현지화, 지원 언어별 README 번역, 그 밖의 다국어 기여 가이드, 눈에 띄는 업데이트 알림이 포함된 업데이트 안내, 프롬프트를 제출하지 않고도 세션의 모델·메모리·컴퓨팅 호스트 설정을 조회하고 업데이트하는 세션 구성 컨트롤을 갖춘 헤드리스 Task API와 CLI, 로컬 진단, 커뮤니티 링크.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
