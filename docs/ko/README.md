...
> 💡 **[Open Science v0.18.2 출시](https://github.com/aipoch/open-science/releases/latest)** _(마지막 업데이트: 2026년 8월)_. Open Science v0.18.2에서는 러시아어 현지화, Markdown 및 PDF 대화 내보내기의 턴 선택, 암호화된 비밀 저장과 발급자 고정을 지원하는 사용자 지정 커넥터용 사전 등록 OAuth 클라이언트를 추가했습니다. 제한된 이벤트 수용과 프로젝트 아티팩트 ZIP 스트리밍으로 사용량이 많은 작업 공간의 응답성을 유지하며, Notebook 실행에는 진행 상태 하트비트와 UTF-8 소스 디코딩을 도입했습니다. 메시지 편집 시 첨부 파일이 보존되고, 불투명한 Codex 재개 실패가 복구되며, Task API 재시도가 멱등성을 보장하고, 작업 공간·업데이트·영속성 관련 여러 문제가 수정되었습니다. 자세한 내용은 [최신 릴리스 노트](https://github.com/aipoch/open-science/releases/latest)를 확인하세요.
...
| **모델**              | 기본 클라우드 제공업체, 사용자 지정 호환 게이트웨이, Claude 및 Codex 구독 로그인, 연결 검증, 모델별 멀티모달 이미지 입력, 모델과 지원 추론 강도를 합친 세션별 작성창 선택기, 새 세션에만 쓰이는 설정 기본값, 텍스트 전용 백엔드를 위한 영구 이미지 증거 릴레이가 있는 전용 Vision 모델 선택기. 제공업체와 API 형식은 선택한 에이전트 백엔드에 맞춰 검증됩니다.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
