...
Open Science는 과학자와 연구자를 위한 오픈 소스 로컬 우선 AI 연구 워크벤치입니다. 과학 AI 에이전트, Python 및 R 실행, 과학 데이터 커넥터, macOS·Windows·Linux 크로스 플랫폼 지원을 통해 모델 전반에서 재현하고 검토할 수 있는 연구를 수행합니다. 하나의 워크스페이스에서 프로젝트를 만들고 연구 목표를 자연어로 설명하면, 에이전트가 파일을 읽고 웹을 검색하며 코드를 실행하고 과학 데이터 소스를 조회하여 추적 가능한 출처가 포함된 보고서, 표, 그림을 생성합니다.
...
> 💡 **[Open Science v0.18.0 출시](https://github.com/aipoch/open-science/releases/latest)** _(마지막 업데이트: 2026년 8월)_. Open Science v0.18.0은 즐겨찾기와 드래그 정렬을 갖춘 리소스 간 카탈로그 태그, 재설계된 스킬 관리, 캐시 즉시 로딩·기능 아이콘·외관 빠른 편집을 갖춘 더 빠른 스페셜리스트 마켓플레이스를 도입합니다. 컴퓨팅 호스트는 OS 암호화 자격 증명을 사용하는 SSH 비밀번호 인증을 지원하며, 한국어 현지화와 다국어 README 및 기여 가이드로 접근성을 넓혔습니다. 이 릴리스는 마켓플레이스와 커넥터 로딩, 런타임 이벤트 스트리밍, 보호된 삭제, 온보딩 원자적 저장, 시작·세션·컴퓨팅·업데이트 복구도 개선합니다. 자세한 내용은 [최신 릴리스 노트](https://github.com/aipoch/open-science/releases/latest)를 확인하세요.
...
| **배포 및 지원**      | macOS, Windows, Linux 설치 프로그램, 환경·에이전트 런타임·모델 제공업체·Notebook 런타임·데이터 위치를 위한 간결한 최초 실행 마법사, 프랑스어·중국어 간체·중국어 번체·일본어·한국어 인터페이스와 일치하는 다국어 README 및 기여 가이드, 눈에 띄는 업데이트 알림이 포함된 업데이트 안내, 로컬 진단, 커뮤니티 링크.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
