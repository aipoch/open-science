...
> 💡 **[Open Science v0.23.0 已發佈](https://github.com/aipoch/open-science/releases/latest)** _（最後更新於 2026 年 8 月）_。Open Science v0.23.0 讓智能體閱讀你正在閱讀的論文：可將最多三個 PDF 連結至會話作為閱讀上下文，讓智能體讀取、翻閱並搜尋這些文件，並可將文字或區域醒目標示為證據，在來源文件中點按即可顯示——同時還有 Notebook 中的即時核心變數建議與停駐式變數面板、Tencent Coding Plan 與 Token Plan 訂閱、會跟隨所設定儲存位置的 Notebook 工作負載快取，以及一系列儲存完整性、遠端存取與憑證修復。詳情請參閱[最新版本說明](https://github.com/aipoch/open-science/releases/latest)。
...
| **執行**           | 具持久程式碼/輸出歷史的 Python、R 與 REPL 控制平面核心，以及記錄於同一執行歷史中的無狀態命令列指令；用於智能體驅動評估的有界 REPL 推理；支援離線佈建的應用程式管理環境；自備 Python 與 R 直譯器；作為額外執行目標的遠端 SSH 運算主機，支援金鑰或密碼驗證（含 Windows）並使用作業系統加密儲存憑證；與智能體共用的使用者終端，輸入時可即時取得來自執行中核心的變數名稱建議；每個執行環境的唯讀已安裝套件清單；執行中 Python 和 R 核心的唯讀即時變數瀏覽器；供智能體端檔案檢查的 Notebook 產物讀取；含耗時顯示的套件安裝進度；以及長時間執行 Notebook 的漸進式歷史載入。外部 R 執行環境的套件管理仍需手動完成。                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
...
| **發佈與支援**     | macOS、Windows 及 Linux 安裝程式；針對環境、資料位置、智能體執行環境、模型服務商與 Notebook 執行環境的精簡首次啟動引導；支援西班牙文、法文、簡體中文、繁體中文、日文、韓文、巴西葡萄牙文和俄文介面，為每種支援語言提供 README 譯本，另有多語言貢獻指南；具醒目更新提醒的更新指引；本地診斷；社群連結。                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
