...
| **文獻**           | 可直接用於專案的文獻庫：以識別碼（DOI、PubMed ID、arXiv ID 等）或從檔案匯入題錄，將它們整理成文獻集、連結到專案，並讓下載的 PDF 經由收件匣處理，且可從垃圾桶復原；透過公開的開放取用來源（Europe PMC、PMC、OpenAlex 與 Unpaywall）尋找並附加全文 PDF，可視需要提供憑證；並排比較重複題錄並批次合併，同時保留附件與關聯；以及依已儲存的中繼資料格式化引用，並附上產物溯源。                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
