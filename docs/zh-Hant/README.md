...
> 💡 **[Open Science v0.24.0 已發佈](https://github.com/aipoch/open-science/releases/latest)** _（最後更新於 2026 年 8 月）_。Open Science v0.24.0 讓你在專案之間移動更快速，執行程式碼也更安全：工作區選單中的專案快速切換器、僅限於你核准網域的 Notebook 與運算網路存取（新網域可在對話中核准）、自訂連接器可繫結的裝置層級共用憑證、可一鍵還原的安全預設權限，以及德文介面——同時還有 Notebook 與運算執行的不可變檔案世代、加固的遠端運算工作，以及一系列執行環境、連接器與儲存方面的修復。詳情請參閱[最新版本說明](https://github.com/aipoch/open-science/releases/latest)。
