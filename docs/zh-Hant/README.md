...
| 領域                           | 核心能力                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| ------------------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **專案與會話**                 | 建立和組織專案，支援會話置頂、持久訊息分支和側邊對話以及可編輯會話詳情。可將已完成的提示編輯為持久、可選取的訊息分支，而不刪除原有後續路徑，並復原近期工作、草稿、對話歷史和預覽狀態。側聊以獨立預覽分頁開啟——可在仍可互動的主對話旁同時執行多個側聊，以繼承目前模型的草稿啟動，並在兩者之間搬移註解。可將會話匯出為自包含的 `.science` 研究套件，並匯入到另一個專案或機器——對話分支、檔案版本、Notebook 記錄與驗證證據都一併隨行。                                                                                                                                                                                                                  |
| **智能體工作流程**             | 在自然語言會話中取得串流回應和依用途分組的工具活動，並使用核准、停止、排隊跟進、上下文壓縮及重新啟動復原。可將已完成工作分支為新會話，並結合結構化澄清、文字/圖片/PDF 註解、關聯 PDF 閱讀上下文、專案記憶、會話引用及需審閱的計畫，以及跨重新啟動保留的對話文字、預覽與 PDF 段落私人書籤（可附註記）。分類式全域搜尋可觸及專案、會話、訊息內文、上傳與生成的檔案及文獻庫，並附標準與進階篩選器以及可跳至符合訊息的情境詳細預覽。通知、即時狀態、耗時與詞元詳情、命令面板、來源預覽和專案切換讓長時間研究保持可見、可管理。                                                                                                                           |
| **模型與智能體後端**           | 使用內建雲端服務商，包括 Apodex、提供精選智能體模型目錄的 NVIDIA Build，以及最新的 OpenAI 和 Anthropic 模型目錄（GPT-6 Astra 和 Claude Fable 5.1）；也可連接自訂相容閘道，或使用 Claude 和 Codex 訂閱登入。可選擇 Claude Code、OpenCode、Codex 或無需登入的 CodeBuddy 作為智能體後端，並取得模型和 API 相容性驗證、多模態圖片輸入、推理強度控制，以及獨立的子智能體、審查者和 Vision 策略。應用程式管理的執行環境可就地更新——受管理的 Codex CLI 可從設定中取得經測試的升級，並分別顯示 CLI 與轉接器版本。                                                                                                                                            |
...
| **Python、R、Notebook 與 HPC** | 執行持久化 Python、R 和 REPL 核心以及納入同一歷史記錄的命令列指令，可使用受管理離線環境或自備直譯器。在本機工作，或透過 SSH 連接遠端主機並使用 Slurm 向 HPC 叢集提交 Notebook 工作；受保護的網路存取、加密憑證、套件與變數檢視、共用終端和漸進式歷史載入讓運算過程可控且可觀察。長時間的 Notebook、REPL 與命令列工作可在背景執行——在釋放智能體輪次的同時保留確切的執行識別、取消與溯源，並在本機執行與遠端運算作業之間自動交付結果。在 Windows x64 上，可選用的 WSL2 Bash 預覽會將 Shell 指令交由通過就緒檢查的 WSL2 環境執行。自備的 R 執行環境可將缺少的套件安裝到經同意核准的個人程式庫，已擷取的相依鎖定則可匯出還原指令碼，用於條件式套件還原。 |
| **文獻回顧與參考文獻管理**     | 透過 DOI、PubMed ID、arXiv ID 或檔案匯入參考文獻——單一 PDF 透過中繼資料編輯器，或一次匯入多個檔案並提供逐檔進度、重複處理與重試——並可一眼掌握目前文獻庫的參考文獻總數；整理文獻集、將參考文獻關聯到專案，並可從垃圾桶復原下載的 PDF。平行檢索 Europe PMC、PMC、OpenAlex、arXiv 和 Unpaywall 的開放取用全文，依 DOI 查詢出版者中繼資料與資料集連結，在不遺失附件或連結的情況下合併重複記錄，並依已儲存中繼資料產生附產物溯源的引用。可在本機從 PDF 擷取圖片與表格——重建的裁切影像、圖說與合併表頭——並可複製或匯出 HTML、TSV、Markdown 研究表格，或讓智能體直接列出並讀取已連結 PDF 中擷取的圖、表與演算法。                                           |
...
| **產物與溯源**                 | 保存不可變、限定於會話的產物版本，包含內容總和檢查碼、生成程式碼、執行歷史、確切輸入、環境清單、訊息分支上下文、沿襲關係和審查證據。任何已擷取的版本都能驗證：在隔離環境中重播其密封配方，並將重現的檔案與原始檔案比較——包含位元組精確檢查、有界的圖像與表格比較、選用的科學規則，以及可匯出的驗證記錄。可編輯的 Markdown、文字、指令碼和原始碼每次儲存都會發布保留溯源的新版本，並可與前一版本比較。                                                                                                                                                                                                                                                |
| **科學技能與資料連接器**       | 使用 **22 個精選**內建技能和 **24 個內建**研究連接器擴充研究工作流程。可透過對話或已完成工作建立技能，匯入套件和 GitHub 來源，並新增具有工具層級權限及設定匯入/匯出能力的自訂本機或遠端 MCP 連接器。可透過簽章目錄瀏覽經驗證的技能市集——依分類與作者探索技能，支援一鍵安裝、版本更新，以及具篩選選取與可重試進度的批次安裝——並可從設定中批次管理連接器。核心應用程式技能一律保持啟用，讓內建入口持續可用；無介面 CLI 與 Task SDK 也能列出、檢視並啟用或停用連接器。跨資源標籤、受保護的「收藏夾」標籤和可搜尋篩選器用於組織技能、連接器和專家。                                                                                                      |
| **本機資料、隱私、權限與驗證** | 將專案資料、應用程式狀態和 Notebook 快取保存在可設定、可移轉的本機儲存空間中；支援系統、手動或直接連線 Proxy 模式，以及包含 30 天活動熱圖和逐次執行歸因的詞元儀表板。透過 `Ask for approval`、`Auto-approve edits` 或 `Full access`、限定範圍的授權、集中式憑證（無介面 Linux 部署可使用明確的檔案儲存模式）、使用者核准的運算網域以及連接器和工具層級原則控制操作。模型輸出中的遠端媒體僅在明確啟用後才會載入，傳送給模型的圖片會被標準化，遠端模型端點則必須使用 HTTPS。可選審查者會稽核對話記錄、執行記錄檔和產物，回報通過/警告/失敗結果，並可執行保留持久證據的有界修正迴圈。                                                                   |
