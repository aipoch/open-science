...
> 💡 **[Open Science v0.21.0 已發佈](https://github.com/aipoch/open-science/releases/latest)** _（最後更新於 2026 年 8 月）_。Open Science v0.21.0 新增第四個智慧體框架 CodeBuddy（應用程式管理、無需登入）、可將選取文字與影像作為上下文傳送給智慧體的批註功能、大幅擴充的 OpenCode Go 與 Zen 模型目錄、遠端運算的 Windows SSH 密碼認證、對話時間軸中的智慧體設定變更標記，以及重新設計的通知訊息中心與專家市集；同時強化渲染器權限與連接器逾時處理，並修復大量運算、筆記本、工作階段與工作區問題。詳情請參閱[最新版本說明](https://github.com/aipoch/open-science/releases/latest)。
...
| **執行**           | 具持久程式碼/輸出歷史的 Python、R 與 REPL 控制平面核心，以及記錄於同一執行歷史中的無狀態命令列指令；用於智能體驅動評估的有界 REPL 推理；支援離線佈建的應用程式管理環境；自備 Python 與 R 直譯器；作為額外執行目標的遠端 SSH 運算主機，支援金鑰或密碼驗證（含 Windows）並使用作業系統加密儲存憑證；與智能體共用的使用者終端；每個執行環境的唯讀已安裝套件清單；執行中 Python 和 R 核心的唯讀即時變數瀏覽器；供智能體端檔案檢查的 Notebook 產物讀取；含耗時顯示的套件安裝進度；以及長時間執行 Notebook 的漸進式歷史載入。外部 R 執行環境的套件管理仍需手動完成。                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
